...
//! | FORK CHOICE | **NOT DECIDED BY THIS OBSERVER**, and that is now a statement about scope rather than about the tree. ⚑ CHANGED 2026-07-30: Samasika chain selection IS formalized, exported and running — `Dregg2.Bridge.MinaForkChoiceGate` (`dregg_mina_better_tip`), fed by the Lean binprot decoder `Dregg2.Bridge.MinaBinprot` off the peer-to-peer wire, driven by [`crate::mina_head::MinaVerifiedHead`]. What THIS module decides is still only an anchored segment: an accept says "this exhibited segment is anchored, linked, canonical and `k` deep", not "and it is the chain the network selected". Two `k`-deep segments under different anchors are still indistinguishable *here* — the object that distinguishes them is `mina_head`, and it does not read `bestChain`. |
//!
...
//! (trusted config), and fork choice. ⚑ That last one is no longer "formalized nowhere":
//! as of 2026-07-30 Samasika's `select` is proven, `@[export]`ed and running against
//! binprot protocol states pulled over Mina's own peer-to-peer RPC — see
//! [`crate::mina_head`] and `docs/MINA-LIGHT-CLIENT.md`. It is simply not THIS module's
//! job. Verifying each block's proof is necessary for a Mina light client and nowhere
//! near sufficient; chain selection is what turns an anchored-segment verifier into a
//! chain follower, and it now lives one module over.
