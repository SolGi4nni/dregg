...
// ⚑ THE BRIDGE-ACTION FOLD-WIRE IS DELETED (2026-08-07).
//
// `BRIDGE_TUPLE_LEN`, `prove_bridge_binding_node` and `prove_bridge_binding_node_segmented`
// folded `bridge_action_air_v1` — a binding-only leaf over a PROVER-CHOSEN tuple. Nothing ever
// enforced that tuple (`verify_bridge_action` never existed anywhere in the tree), the leg side
// it waited on was never emitted, and `Dregg2.Circuit.BridgeBindingFromFold` REFUSES the AIR as
// a bridge-mint backing precisely because connecting a prover-chosen tuple is the vacuous
// connect the fail-open law forbids.
//
// The bridge-mint backing a light client actually folds is the RE-PROVEN note-spend STARK
// connected to the leg's published PI-46 `mint_hash`: `note_spend_leaf_adapter::
// prove_note_spend_leaf_with_claim` + `prove_note_spend_mint_binding_node_segmented` below,
// carried by `CarrierWitness::Bridge` / `BridgeWitnessBundle`. That path is untouched.
