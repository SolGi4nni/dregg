...
//!
//! ## ⚑ THE TOWER IS TWO ENGINES DEEP, AND IT WAS ONE UNTIL 2026-08-08
//!
//! A leaf VERIFIES its IR-v2 child at the child's own minting engine ([`chain_inner_config`],
//! `log_blowup 6 / 19 queries`) and MINTS at the recursion engine ([`chain_root_config`],
//! `log_blowup 3 / 38 queries`), which every fold and the root verify then run at. Both are
//! generated by iterating `dregg_recursion_verify::config::recursion_layer_over`; **no function in
//! this module names a config constant.** Until 2026-08-08 one config played all four roles and
//! played them at the child's `log_blowup 6`, so a 2^20-row wrap trace committed a 2^26 LDE
//! domain where 2^23 was the correct one. ⚠ **Every chain-root VK rotates**; an operator's
...
///
/// ⚑ `config` is the engine the INNER descriptor batch is minted at, and therefore the engine this
/// leaf's in-circuit verifier checks it against. The WRAP's own minting engine is **derived** from
/// it by [`recursion_layer_over`] — it is not this config and it is not a named constant, which is
/// exactly how the predecessor of this change was inert while every config object looked right.
...
///
/// [`prove_chain_link_leaf_with`] derives the second from the first. This function exists so the
/// OLD single-engine shape stays REACHABLE for a before/after measurement in one binary — passing
/// `inner_config` twice is what this tower did until 2026-08-08 — and for nothing else. Production
/// callers want the deriving entry point; a caller that names a wrap engine here is asserting one.
pub fn prove_chain_link_leaf_split(
    desc: &EffectVmDescriptor2,
    trace: &[Vec<BabyBear>],
    public_inputs: &[BabyBear],
...
///
/// ⚠ `config` here is the FOLD engine — [`chain_root_config`], the fixed point of
/// [`recursion_layer_over`] — **not** the inner descriptor engine a leaf is handed. Both are
/// `DreggRecursionConfig` and nothing in the type system separates them; passing the leaf's config
/// here is a `QueryProofCountMismatch { expected: 19, got: 38 }` at the in-circuit verifier.
/// [`prove_chain_fold`] derives it so no caller has to remember.
...
///
/// ⚑ `config` is the INNER (IR-v2 descriptor) engine — [`chain_inner_config`]. The leaf wrap
/// engine and the fold engine are both DERIVED from it here; a caller never names either.
///
...
pub fn prove_chain_fold_with(
    ...
    // ⚑ THE FOLD ENGINE, DERIVED. A chain leaf now emits at `recursion_layer_over(config)`'s mint
    // knobs, so every node above it verifies at those — one more application of the same rule,
    // which lands on its fixed point. Deriving it is what keeps this honest if `config` is retuned.
    ...
    for (i, (trace, pis)) in link_witnesses.iter().enumerate().skip(1) {
        ...
        acc = fold_chain_links(&acc, &leaf, &pins, &fold_config)?;
    }
    Ok(acc)
}

/// ⚑ **THE ENGINE THE CHAIN LINK'S INNER DESCRIPTOR BATCH IS MINTED AT** — and therefore the engine
/// the leaf wrap's in-circuit verifier checks it against. Pass this to [`prove_chain_link_leaf`] /
/// [`prove_chain_fold`]; the wrap and fold configs are derived from it by [`recursion_layer_over`].
///
/// ⚠ It is **NOT** the config a root verifies under — that is [`chain_root_config`]. The single
/// `chain_config()` that used to answer both questions is DELETED rather than kept as an alias: it
/// was only ever right because the two answers coincided, and a name that silently means the wrong
/// one of a pair is how a GPU dispatch minted at a constant for two months.
pub fn chain_inner_config() -> DreggRecursionConfig {
    ir2_leaf_wrap_config()
}

/// ⚑ **THE ENGINE EVERY LAYER ABOVE A CHAIN LEAF RUNS AT** — all 45 folds, and the native verify of
/// the root. The fixed point of [`recursion_layer_over`].
///
/// ⚑ RE-EXPORTED, not redefined, for the reason `mina_accumulator_fold` re-exports its own: a
/// fold config is exactly the kind of thing that drifts silently between a prove side and a verify
/// side — both would still BUILD, and only the FRI would disagree.
pub use dregg_recursion_verify::chain_root::chain_root_config;
