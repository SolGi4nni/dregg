...
/// The wire string Lean's `emitVmJson2 committedThresholdDesc` emits, READ FROM THE EMITTED
/// ARTIFACT (routed by `EmitByName.lean`, drift-gated, stamped). Was a hand-copied literal until
/// 2026-08-06; the `#guard` in `CommittedThresholdEmit.lean` and the copy each pinned only
/// themselves. No copy now — the artifact IS the Lean emitter's output.
const GOLDEN_JSON: &str =
    include_str!("../../circuit/descriptors/by-name/committed-threshold.json");
