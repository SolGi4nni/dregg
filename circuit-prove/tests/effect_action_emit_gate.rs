...
// ── The wire strings Lean's `emitVmJson2` emits, READ FROM THE EMITTED ARTIFACTS (routed by
//    `EmitByName.lean`, drift-gated, stamped). These were TWO hand-copied literals in ONE file
//    until 2026-08-06 — the `#guard`s in `EffectActionBindingEmit.lean` pinned Lean, the copies
//    pinned themselves, and nothing re-derived either from the other. No copies now. ──
const REVOKE_GOLDEN: &str =
    include_str!("../../circuit/descriptors/by-name/effect-revoke-capability.json");
