...
/// The wire string Lean's `emitVmJson2 foldDesc` emits, READ FROM THE EMITTED ARTIFACT (routed
/// by `EmitByName.lean`, drift-gated by `scripts/check-descriptor-drift.sh`, stamped in
/// `PROVENANCE.json`). This was a hand-copied `r#"…"#` literal until 2026-08-06: the `#guard` in
/// `FoldEmit.lean` pinned the Lean side, this pinned the Rust side, and NOTHING re-derived one
/// from the other, so a Lean move left the copy certifying its own stale bytes. There is no copy
/// now — if the emission moves, this file moves with it.
const GOLDEN_JSON: &str = include_str!("../../circuit/descriptors/by-name/fold-step.json");
