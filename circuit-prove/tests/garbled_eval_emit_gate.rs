...
/// The wire string Lean's `emitVmJson2 garbledEvalDesc` emits, READ FROM THE EMITTED ARTIFACT
/// (routed by `EmitByName.lean`, drift-gated, stamped). Was a hand-copied literal until 2026-08-06;
/// the `#guard` in `GarbledEvalEmit.lean` and the copy each pinned only themselves. No copy now.
const GOLDEN_JSON: &str = include_str!("../../circuit/descriptors/by-name/garbled-eval.json");
...
fn hand_built_desc() -> EffectVmDescriptor2 {
    ...
    // C9-C23: decryption correctness (gated on 1 - is_padding), the six boolean selectors, and
    // gate-type exclusivity. Held in their own vec because `aca1a0cbc` re-asserts every one of
    // them as a LAST-ROW boundary too (see below), and a second hand-transcription of the same
    // fifteen bodies is a second place for them to drift.
    let mut row_local: Vec<VmConstraint2> = Vec::new();
    for i in 0..8 {
        row_local.push(dec_gate(i));
    }
    row_local.push(bin_gate(IS_AND));
    row_local.push(bin_gate(IS_OR));
    row_local.push(bin_gate(IS_XOR));
    row_local.push(bin_gate(IS_NOT));
    row_local.push(bin_gate(CHAIN_FLAG));
    row_local.push(bin_gate(IS_PADDING));
    row_local.push(exclusivity_gate());
    constraints.extend(row_local.iter().cloned());
    ...
    // LAST-ROW boundary copies of C9-C23 (`aca1a0cbc`, stark-kill bugfix 8/10). A p3 row-local
    // gate does not constrain the final row, so before this the last row's table_entry output was
    // unconstrained — a forgery slot. Re-asserting each body as a `last` boundary closes it.
    constraints.extend(row_local.into_iter().map(|c| match c {
        ...
        other => panic!("C9-C23 must all be row-local gates, got {other:?}"),
    }));
...
fn garbled_eval_emit_decodes_to_hand_built() {
    ...
    assert_eq!(
        ...
        47,
        "the Lean #guard pins 47 constraints (32 + the 15 last-row boundary copies of C9-C23)"
    );
