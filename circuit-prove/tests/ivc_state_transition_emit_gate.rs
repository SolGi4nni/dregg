...
/// The wire string Lean's `emitVmJson2 ivcStateTransitionDescriptor` emits, READ FROM THE EMITTED
/// ARTIFACT (routed by `EmitByName.lean`, drift-gated, stamped). Was a hand-copied literal until
/// 2026-08-06; the `#guard` in `EffectVmEmitIvcStateTransition.lean` and the copy each pinned only
/// themselves. No copy now.
const GOLDEN_JSON: &str =
    include_str!("../../circuit/descriptors/by-name/ivc-state-transition.json");
