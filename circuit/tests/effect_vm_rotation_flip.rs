...
fn rotated_transfer_proves_verifies_differential_and_refuses_ghost() {
    ...
    assert_eq!(
        dpis[dregg_circuit::effect_vm::trace_rotated::V1_PI_COUNT],
        r0[before_base + B_STATE_COMMIT],
        "PI V1_PI_COUNT (was 42) = rotated OLD commit"
    );
    assert_eq!(
        dpis[dregg_circuit::effect_vm::trace_rotated::V1_PI_COUNT + 1],
        last[after_base + B_STATE_COMMIT],
        "PI V1_PI_COUNT+1 (was 43) = rotated NEW commit"
    );
    assert_eq!(
        dpis[dregg_circuit::effect_vm::trace_rotated::V1_PI_COUNT + 2],
        last[after_base + B_COMMITTED_HEIGHT],
        "PI V1_PI_COUNT+2 (was 44) = committed height"
    );
    assert_eq!(
        dpis[dregg_circuit::effect_vm::trace_rotated::V1_PI_COUNT + 3],
        last[caveat_base + C_CAVEAT_COMMIT],
        "PI V1_PI_COUNT+3 (was 45) = caveat commit"
    );
...
fn rotated_note_spend_pins_nullifier_and_refuses_tamper() {
    ...
    assert_eq!(
        ...
        "PI ROT_PI_COUNT (was 46) = the spend row's folded nullifier (param0)"
    );
...
fn rotated_create_cell_pins_accounts_and_refuses_tamper() {
    ...
    assert_eq!(
        dpis[dregg_circuit::effect_vm::trace_rotated::ROT_PI_COUNT],
        trace[0][PARAM_BASE],
        "PI ROT_PI_COUNT (was 46) = the create row's new-cell key (param0)"
    );
...
fn rotated_set_field_and_bridge_mint_tick_nonce_and_refuse_forged_delta() {
    ...
    // ===== (A) ROTATED SETFIELD (slot 0): tick nonce, prove+verify, refuse forged delta. =====
    {
        ...
        //   * the VALUE8 completion pins went 7 -> 8 (PI ROT_PI_COUNT (was 46)..=52 -> 46..=53) — the ninth lane's
...
fn rotated_cellseal_record_pin_forces_lifecycle_and_rejects_frozen_forgery() {
    ...
    assert_eq!(
        ...
        "PI ROT_PI_COUNT (was 46) = the AFTER block's correctly-written (sealed) lifecycle limb"
    );
    assert_eq!(
        dpis[dregg_circuit::effect_vm::trace_rotated::ROT_PI_COUNT],
        after_w.pre_limbs[B_LIFECYCLE],
        "PI ROT_PI_COUNT (was 46) = lifecycle_felt(Sealed) from the post-state producer witness"
    );
    // The four commit pins are undisturbed below it.
    assert_eq!(
        dpis[dregg_circuit::effect_vm::trace_rotated::V1_PI_COUNT],
...
fn rotated_audit_record_pin_forces_record_digest_and_rejects_frozen_forgery() {
    ...
    for (effect, expect_name, pin_limb) in [
        ...
        assert_eq!(
            ...
            "PI ROT_PI_COUNT (was 46) = the AFTER block's correctly-written audit limb (r23 record-digest for refusal, \
             lifecycle_felt for archive)"
        );
        assert_eq!(
            dpis[dregg_circuit::effect_vm::trace_rotated::ROT_PI_COUNT],
            after_w.pre_limbs[pin_limb],
            "PI ROT_PI_COUNT (was 46) = the post-state producer witness's pinned limb"
        );
        if exact_refusal {
...
///   1. an HONEST fee'd transfer PROVES + VERIFIES, and NEW_COMMIT (PI V1_PI_COUNT+1 (was 43)) binds the POST-fee
///      balance (the verifier reads the fee off PI ROT_PI_COUNT (was 46), not off any trusted ledger value);
...
fn fee_debit_is_proven_and_underclaimed_fee_is_unsat_for_a_ledgerless_client() {
    ...
    assert_eq!(
        ...
        "PI ROT_PI_COUNT (was 46) is the published fee"
    );
    ...
    // And NEW_COMMIT (PI V1_PI_COUNT+1 (was 43)) is the trace's last-row after-block STATE_COMMIT carrier (post-fee).
    assert_eq!(
        dpis[dregg_circuit::effect_vm::trace_rotated::V1_PI_COUNT + 1],
    ...
    //    forces col89 = PI[46]. So publishing a smaller fee cannot satisfy the descriptor. --
    {
        ...
        // the published PI ROT_PI_COUNT (was 46) with the underclaim. The bal-lo gate (`after = before − amount − col89`)
    ...
    eprintln!(
        "FEE-IN-PROOF CLOSED (R=24, LIVE): the deployed sovereign transfer debits the fee INSIDE \
         ...
         − fee`, fee pinned to PI ROT_PI_COUNT (was 46)). An honest fee'd transfer PROVES+VERIFIES with NEW_COMMIT \
...
// ⚑ DELETED 2026-07-31 — `wide_transfer_proves_verifies_and_the_high_position_collision_tooth_bites`
// (267 lines), together with its subject: the committed one-row fork
// `circuit/descriptors/rotation-wide-transfer-staged.tsv` (`transferVmDescriptor2R24Wide`) and the
// producer `trace_rotated::generate_rotated_transfer_wide`, neither of which had a production
// consumer.
//
// ⚠ SAY WHAT THIS COSTS. This test carried TWO real things — the first Plonky3 prove+verify at the
// wide geometry, and the 8-felt high-position collision tooth (two states differing only in a byte
// of `fields[15]` publish different commits; a proof for A is REJECTED against B's commit). Both
// were demonstrated on a descriptor that NO verifier resolves: the deployed `transferVmDescriptor2R24`
// (`WIDE_REGISTRY_STAGED_TSV` row 0) is availability-hardened, membership-teeth-advanced,
// gentian-refuse-welded and E1-compacted — 1782 cols / 68 PIs against the fork's 1819 / 66, with 560
// of the fork's 619 constraints appearing nowhere in it.
//
// So the tooth was not lost here; it was never pointed at the shipped member. Deleting the fork makes
// that visible. The remaining work is to re-mint this collision tooth on the deployed row 0 through
// the avail-shifted, refuse-welded wide producer
// (`generate_rotated_effect_vm_descriptor_and_trace_wide`) — the sibling roundtrips in
// `circuit/tests/effect_vm_wide_roundtrip.rs` already run that way for burn / noteSpend / noteCreate /
// createCell, so transfer is the one shape missing from that file, not from the suite's design.
