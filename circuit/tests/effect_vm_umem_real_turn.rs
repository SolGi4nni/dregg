...
// ============================================================================
// DELETED 2026-08-06 — the eight BOUND whole-image-fold tests (295 lines):
//   `whole_image_fold_bound_{proves_against_boundary_table, undeclared_cell_refuses,
//    boundary_value_mismatch_refuses, smuggled_start_root_refuses}` and their flat-memory twins
//   `whole_image_fold_bound_mem_{proves_against_boundary_table, forged_minit_refuses,
//    undeclared_cell_refuses, smuggled_start_root_refuses}`.
//
// The subjects they exercised are gone: `whole_image_fold::*_bound*` is DELETED. The reason is
// in that module's banner, and it is the reason these tests could not have caught it — MEASURED
// 2026-08-06:
//
//   * `verify_vm_descriptor2` assigns public values to the MAIN instance ONLY
//     (`pvs = [public_inputs]; pvs.resize(airs.len(), vec![])`). The umem / flat-mem BOUNDARY
//     instance therefore received an EMPTY public-value vector — the verifier had no handle on
//     its contents at all, and the bound wrappers' public inputs were byte-identical to the
//     unbound chip's `[empty_root8, published_root8]`.
//   * So all six `*_refuses` assertions above split into two kinds, and NEITHER was a
//     verifier-side binding: the three boundary teeth (undeclared cell, value mismatch, forged
//     `minit`) refused inside `prove_whole_image_fold_bound*`, i.e. an honest prover handed a
//     boundary inconsistent with its own fold list declined to build a trace — while a real
//     adversary supplies both and simply makes them agree; and the two `smuggled_start_root`
//     teeth refused in `assert_empty_root_pin`, which is the UNBOUND wrapper's check and is
//     still exercised by `cross_cell_read_whole_image_folds_to_published_root` and friends above.
//
// The UNBOUND chip's teeth are unaffected and remain verifier-visible (both root PI groups are
// pinned, plus the empty-root pin). `circuit/tests/whole_image_fold_lean_correspondent.rs` still
// pins the fold's shape to `Dregg2.Circuit.WholeImageFoldRealization.wholeBoundaryFold8`.
// ============================================================================
