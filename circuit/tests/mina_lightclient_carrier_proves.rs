...
//! wraplink program whose public-input commitment it published. What that sub-proof establishes is
...
const WRAPLINK_DESCRIPTOR: &str = "dregg-pasta-fq-wraplink::v1";
...
/// `LightClientMinaAir.WRAPLINK_VK_LANES` — the nine `Faithful9` lanes of the wraplink descriptor's
...
const WRAPLINK_VK_LANES: [u32; 9] = [
    460719650, 491018495, 181487667, 118226689, 312849261, 6346964, 450127067, 318052623, 12172205,
];
/// `LightClientMinaAir.WRAPLINK_PI_LANES` — the nine lanes of the digest of the wraplink
/// sub-proof's 224 public inputs on the block-539508 instance.
const WRAPLINK_PI_LANES: [u32; 9] = [
...
fn honest_cells() -> Vec<i64> {
    ...
    for i in 0..STATE_LANES {
        ...
        c[SUB_VK_0 + i] = i64::from(WRAPLINK_VK_LANES[i]);
        c[SUB_PI_0 + i] = i64::from(WRAPLINK_PI_LANES[i]);
    }
    c[WRAP_FS_PROVED] = 1;
    c
}

/// The public inputs the row's twenty-nine pins publish, read OFF THE ROW so a tampered column and
...
fn the_served_descriptor_has_the_recursion_shape() {
    ...
    let link = descriptor_by_name(WRAPLINK_DESCRIPTOR)
        .expect("the Fq-transcript sub-proof descriptor must dispatch (fail-closed otherwise)");
    assert_eq!(link.public_input_count, 224);
}
...
/// recursion binds to the real wraplink fingerprint.
#[test]
...
fn a_row_naming_a_different_program_is_refused() {
    ...
    let e = refusal::must_refuse(
        "a head row attesting a program that is not the wraplink AIR",
        || prove_and_verify(&desc(), &cells, &pis),
    );
...
/// publishes a different sub-proof commitment (so it can hand over some OTHER verifying wraplink
/// proof) is refused by the PI binding.
#[test]
fn publishing_a_commitment_the_row_does_not_hold_is_refused() {
    let cells = honest_cells();
    let mut pis = pis_of(&cells);
    pis[PI_SUB_COMMIT_BASE] = felt(i64::from(WRAPLINK_PI_LANES[0]) + 1);
    refusal::must_refuse(
...
fn the_named_sub_proof_proves_from_the_served_descriptor() {
    let d = descriptor_by_name(WRAPLINK_DESCRIPTOR).expect("served");
...
fn a_tampered_transcript_element_is_refused_by_the_pin() {
    ...
    // PI block 3 (limbs 96..127) is the absorbed element `LINK_X`.
    let sk = 32usize;
    pis[3 * sk] = BabyBear::new((pis[3 * sk].as_u32() + 1) % 256);
    refusal::must_refuse(
        "a wraplink proof whose absorbed transcript element was moved",
        || {
            let proof = prove_vm_descriptor2(&d, &trace, &pis, &MemBoundaryWitness::default(), &[])
                .map_err(|e| format!("prover refused: {e}"))?;
            verify_vm_descriptor2(&d, &proof, &pis).map_err(|e| format!("verifier refused: {e:?}"))
...
fn a_tampered_transcript_moves_the_published_commitment() {
    let mut raw: Vec<u32> = LINK_PIS
        .split_whitespace()
        .map(|c| c.parse::<u32>().unwrap())
        .collect();
    let honest = wraplink_pi_commitment(&raw);
    raw[3 * 32] = (raw[3 * 32] + 1) % 256;
    let tampered = wraplink_pi_commitment(&raw);
    assert_ne!(
        honest, tampered,
...
/// `dregg_turn::executor::mina_head_verifier::wraplink_pi_commitment`, which
/// `mina_transcript_carrier_binding.rs` gates against the Lean literal.
fn wraplink_pi_commitment(pis: &[u32]) -> [u8; 32] {
    let mut h =
        blake3::Hasher::new_derive_key("dregg.mina-lightclient.wraplink-subproof-pi-commitment.v1");
