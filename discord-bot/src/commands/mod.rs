...
// refuel signal, run-credit inspect/grant). Folded into `/dregg` as a subcommand GROUP rather
// than taking a top-level slot; `/dregg` is the `Door::Operator` row of
// `menus::SLASH_SURFACE`, so Discord hides the whole command from members who cannot use it.
// Every subcommand is gated on `Config::is_admin`, structurally (see `admin::plan`).
pub mod admin;
