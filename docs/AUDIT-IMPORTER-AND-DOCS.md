...
| **3** | **F-A1 + F-B10** — the two best results in this scope cannot go red | `OTPoC.lean:1104-1129`; `bridge/mina-zkapp/**` | Turn the three `pathExists` fallbacks into `throwError`, and wire **one** invocation of each into `scripts/local-gates.sh`. No fallback, green or bust. **[DONE 2026-07-28 for the mina half]** — `scripts/check-mina-attestation.sh`, two `local-gates.sh` rows + a `ci.yml` job, no-skip, with an 8-fault self-test that was watched going red. |
...
| **10** | **F-B6/F-B9** — "runnable PoC", "a Mina zkApp verifies" | commit `a0c384365`; `MINA-DREGG-ZKAPP-BRIDGE.md:18,206,265,270` | As committed it exits 7 on the pinned o1js, and `tsc --noEmit` fails on the committed `.ts`. A **ZkProgram** ran; the `SmartContract` never did. Either pin o1js 2.x or say the PoC runs off-pin. **[DONE 2026-07-28]** — pinned `o1js` **2.15.0** exactly (plus `useDefineForClassFields: false`, without which o1js's `@state` decorator never binds and the zkApp could not have run at all); `tsc` clean; the duplicate `attestation-poc.mjs` deleted so the gate drives `src/`; and `DreggAttestedGate` now deploys with proofs enabled, consumes the attestation proof recursively, and refuses one bound to another root. **F-B7 is answered in-source** (`src/rust-gold-vectors.ts` states the provenance). **F-B8 is NOT closed** — the root is still a `[1,2,3,4]` KAT, now said so in the module header. |
