...
### 1.3 The producer/emit bridge — SUPERSEDED by derivation (Phase 2 closed)

This section described a `RotatedLayoutBridge.lean` that proved each deployed emit `*GroupCol` def
**equals** `rotated178.groupCol`'s projection at every lane — seven `*_eq_layout` theorems, each by
`fin_cases i <;> native_decide` — so that `rotated178` was the **proven model** of the emit's
positions without moving a single emitted byte.

⚑ That file is gone, and the reason is that the stronger property landed: the emit no longer
hand-carries positions to be proven equal, it DERIVES them.
`metatheory/Dregg2/Circuit/Emit/EffectVmEmitRotationV3.lean:150` defines
`layoutGroupCol name i = (rotated178.groupCol name i).getD 0`, and `capRootGroupCol` (`:1492`) and
its six siblings are defined through that. The equalities are `rfl` by construction, so there is
nothing left for a bridge theorem to state. Single-source, not a proven mirror.
