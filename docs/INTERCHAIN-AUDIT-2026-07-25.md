...
B. CUSTODY (funds move): DreggVault.sol (no admin/upgrade/drain; SP1-STARK-proof + isProvenRoot gated),
   ...
   turn/src/executor/bridge_ledger.rs (the dregg-side mint; double-mint+conservation sound but trusts a caller bool).
C. INTEROP (Hyperlane/LayerZero): DreggProofISM.sol + DreggDVN.sol (standard-conforming, Nomad-hardened, but
   attest NOTHING today — isProvenMessageRoot=false, fail-closed); dregg-deploy/src/gate.rs = a doc-comment only.
...
- #5 HIGH — bridge/src/midnight_observer.rs:342: re-executing observer trusts RPC GRANDPA finalized-head; mint via
...
- #7 MED — bridge/src/midnight_inclusion.rs:31: Merkle over a BLAKE3 PLACEHOLDER (TODO(mirror-hash)), not real
  Poseidon2 — tautology, binds nothing (STUB, not a live gate).
- #8 LOW — bridge/src/mina.rs:214: Kimchi verify removed as vacuous; now a BLAKE3 tag over the proof's own bytes
...
- #10 MED — bridge/src/solana_mirror.rs:21: legacy threshold-ed25519 federation-oracle mint (superseded by solana_trustless).
- #11 MED — cosmos-settlement/src/verifier.rs: hand-written 2nd Groth16 verifier vs canonical gnark/Solidity —
...
- #14 LOW — bridge/src/solana_feed.rs:209: mainnet evidence feed "DESIGNED not built" → every Solana lock stays
  consensus_verified=false (safely inert).
- #15 INFO — turn/src/executor/bridge_ledger.rs:270: consensus_verified is a caller bool re-verified nowhere here (the
