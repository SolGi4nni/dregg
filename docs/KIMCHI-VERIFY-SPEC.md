...
side we care about (the side Mina calls the "wrap"/Vesta side):

- `G` = **Vesta**. `G::ScalarField` = **Fq** (≈255-bit, modulus q), `G::BaseField`
  = **Fp** (modulus p < q). [S — `verifier.rs:111-116` bounds, `ipa.rs:313-315`]
- The **Fq-sponge** absorbs Vesta base-field (Fp) elements — curve-point
  coordinates — and squeezes Fq scalars. It is instantiated with the *other*
  curve's Poseidon params: `EFqSponge::new(G::other_curve_sponge_params())`
  [S — `verifier.rs:159`].
- The **Fr-sponge** absorbs Fq elements directly (evaluations) and squeezes Fq
  [S — `verifier.rs:278-287`, `plonk_sponge.rs:16-32`].
- All polynomial evaluations, all challenges (β, γ, α, ζ, v, u, the IPA round
  challenges, c), and all scalar arithmetic in the checks below are in **Fq**.
- All commitments, MSMs, and the final IPA equation are **Vesta group operations**
  (point coordinates in Fp).

**[R-ctx]** When this verifier is itself written as a circuit (our K5), the
circuit's native field is the *other* Pasta field. For a Vesta-proof verifier
the circuit lives over Pallas's scalar field = **Fp**: Vesta point ops are then
native-ish (affine coords in Fp), while **all Fq arithmetic is non-native** and
must be emulated (K1). This is the standard Pickles arrangement; the standalone
Rust verifier knows nothing of it.

`shift_scalar` [S — `commitment.rs:273-288`]: for `G = Vesta`, scalar modulus
(q) > base modulus (p), so the `else` branch applies: `x ↦ x − 2^255`
(`MODULUS_BIT_SIZE` of Fq = 255). Used exactly once, when absorbing the combined
inner product into the Fq-sponge [S — `ipa.rs:372`].
...
`plonk_sponge.rs:47-49`); `to_field(endo_r)` maps it to Fq as `k = a·λ + b`
...
evaluation list (C8). *Gadget:* K2 (MSM over SRS Lagrange points, Fq scalars).
...
**Fq-sponge phase** (Poseidon over Fp-state, squeezing Fq):
...
**Fr-sponge phase** (Poseidon over Fq):
...
16. Compute `public_evals` (see C4) and absorb both `[ζ, ζω]` entries
    (`verifier.rs:391-392`).
17. Absorb `ft_eval1` = ft(ζω) from the proof (`verifier.rs:381-382`).
18. Absorb **all proof evaluations** in the fixed order of
    ...
    at step 16 — the "Mina annoyance", `plonk_sponge.rs:60`).
...
*Gadget:* **K3** (PastaPoseidon sponge, both Fq-sponge-over-Fp and
Fr-sponge-over-Fq flavors, plus the 128-bit challenge squeeze + endo map).
...
*Gadget:* K1 (Fq arithmetic, one batch inversion of 2·|public| elements).
...
*Gadget:* K1 (~50 Fq muls). *Cost:* **10²–10³** gates. **[R-est]**
...
*Gadget:* K1 — the PolishToken evaluator is a small stack machine over Fq ops;
...
`verifier.rs:954-955` — ~40–50 Vesta points, Fq scalars.
...
*Gadget:* K2 (Vesta MSM; non-native Fq scalars). *Cost:* ~45 non-native scalar
...
*Gadget:* K1 for the scalar combination (pure Fq: ~10³ muls ⇒ **10³–10⁴**
...
- k·2 challenge-derived scalar muls on (Lᵢ, Rᵢ) — k = log₂(SRS size), e.g.
  **k = 16** for Mina's wrap SRS ⇒ 32 terms;
- the **s-vector MSM `⟨s, G⟩` with 2^k = 65536 terms** — computing `s` itself
  is 2^k Fq muls, and the MSM is 2^k non-native scalar muls;
...
`zk_rows = 3`; public input present and small. Proof side: Vesta (Fq scalars);
circuit side: Fp-native with non-native Fq (K1).
...
2. **Scalar checks (C4, C5, C6, C8-scalar)** — K1. Pure Fq arithmetic:
...
4. **IPA (C9)** — K4, in this order: per-round challenges uᵢ, uᵢ⁻¹ →
   `b_poly`/`b0` → `b_poly_coefficients`/s-vector (2^k Fq muls) → the final
...
Vesta scalar muls** (plus the 2^k-mul s-vector computation). Nothing else is
