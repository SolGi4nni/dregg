...
Bernstein best-case `2⁻¹¹⁷` the sub-Gaussian surrogates could not clear. **§15 (2026-07-13) TIGHTENS this to
`≤ 2⁻¹⁴⁸` AND makes it UNCONDITIONAL** (`mlkem768_decapsFailure_le_delta_unconditional_tight`, on the real `e_total`,
no `PerCoeffHoeffdingTail` hyp) — 8 genuine bits recovered (crude-tangent bound + log-space headroom + the union bit).
- **The one named residual — a MEASURED method wall, NOT arithmetic slack.** §15 minimized the exact-MGF Chernoff over
`s` with the exact per-term MGFs: it ceilings at `≈ 2⁻¹⁶⁵` per-coefficient → `≈ 2⁻¹⁵⁵` after the 768-fold union.
Reaching the exact FIPS δ (`2⁻¹⁶⁴`) needs a per-coefficient tail of `2⁻¹⁷⁴` — **~9 bits BELOW the Chernoff ceiling**.
The true tail IS below that (near-Gaussian `≈ 2⁻¹⁸⁰`); the gap is the **Bahadur–Rao prefactor Chernoff discards at
~15.7σ**, so closing it needs the exact FFT-convolution tail (a different formalization), NOT a tighter
over-approximation. `2⁻¹⁴⁸` is the honest assumption-free ceiling of the exact-MGF route; the file's own header/§12.4
prose (which had called this "arithmetic slack") is corrected to the measured wall.
...
- **Seam 4:** the exact-MGF route now proves `≤ 2⁻¹⁴⁸` UNCONDITIONAL in-kernel (§15); the gap to the exact FIPS δ
  `2⁻¹⁶⁴` is a MEASURED Chernoff-method wall (the Bahadur–Rao prefactor at ~15.7σ; the method ceilings ≈`2⁻¹⁵⁵` after
  the 768-union), NOT arithmetic slack — closing it needs the exact FFT-convolution tail (a different formalization).
  `2⁻¹⁴⁸` is cryptographically negligible, so this is exactness, not security.
