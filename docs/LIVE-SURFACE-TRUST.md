...
| Payments | **REAL-selection** (`e91222144`) | `select_watcher` now returns a real `SignatureWatcher`+`RpcTransferFetcher` on a real-network config and REFUSES the mock on mainnet (`WatcherSelectError::MockOnMainnet`); an empty RPC is a loud `RpcMissing`. Watch-only, no seed. Credits are keyed on the **transaction signature** (`soltx:{sig}`), so the watcher holds no in-RAM cursor and a restart re-credits nothing — see `docs/reference/RESTART-SEMANTICS.md`. Mainnet arming + seed-custody split stay ember-gated. |
