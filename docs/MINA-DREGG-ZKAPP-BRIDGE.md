...
- **[DELETED 2026-07-30.]** The o1js `DreggVerifier.verifyTransition` (`src/verifier.ts`) asserted
  ...
  together with `DreggFederation` itself, `src/bridge-ops.ts`, the deploy script under `scripts/` and
  the interact script under `scripts/` — see §7's deposit/withdraw row for why. The fold the PoC (§6) wires in now
