...
| Mina verifies a dregg Poseidon attestation | **[SUPERSEDED 2026-07-28 — was "a bare `ZkProgram`, not a zkApp, over `[1,2,3,4]`, off-pin".]** A **deployed zkApp on Mina Devnet** (`B62qo54w6YftnPHCXTrcEYDcKYwg8CCeW5wiTsW9x8Tp1Hm5BS5xCeD`) consumed a **recursive** attestation over a root the **Rust side emitted fresh**, and the network **refused** the same proof once the anchored root moved (`Account_app_state_0_precondition_unsatisfied`, block 539522). Still a **commitment attestation, not a proof check**, and the leaves are deploy-time metadata rather than dregg state. `docs/MINA-DEVNET-DEPLOYMENT.md` is authoritative, including on what it does NOT show. |
...
| Does any of this go red on its own? | **Partly — [CORRECTED 2026-07-28; this row read "No … a complete orphan".]** The offline gate is now wired in: `scripts/check-mina-attestation.sh` is a `local-gates.sh` row **and** a `ci.yml` job, both with a `--self-test` that proves it can go red. ⚑ **Still uncovered:** that script is deliberately Node-only ("no cargo, no Lean"), so the Rust probe's own tests — including `sparse_path_folds_through_the_gold_depth2_root`, which pins the *emitting* side of the bridge — run in NO gate. The devnet scripts are not gated and must not be (network, faucet budget, block times). |
