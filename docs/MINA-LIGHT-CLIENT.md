...
| 12 | **`state_hash` re-derivation** | ❌ **not done, anywhere** | `state_hash = Poseidon("MinaProtoState")[previous_state_hash, body_hash]`, and `Body.hash` absorbs `to_input` in an order that is **not** the binprot order (`protocol_state.ml:119-130`). So a tip's own identity comes from the peer's framing. This is why `eh`/`ch` are **supplied** on the gate wire — the one input that is not derived. `previous_state_hash` *is* decoded and is a real parent link. |
...
to one height, the verifier index is trusted config, `state_hash` is not re-derived, and leader
election is not checkable by any verifier at all. A chain of blocks whose proofs are only
