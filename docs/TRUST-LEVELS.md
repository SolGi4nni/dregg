...
the deployed prover's per-fold FRI column is proven at the arity-2
`ir2_leaf_wrap_config()` (`FriLedgerSound.rotatedLeafWrap_ledger_perFoldBits`) and at the
arity-8 `ir2_config` leaf mint (`FriArityTransfer.arity8_perFold_soundness`), where the arity-2 capacity figure
**provably fails** (`FriArityTransfer.arity8_error_not_lt_2e112`); the deployed **commit
column binds below both** (`FriDeployedHeightPairing.deployed_wrap_commitBits`; digits in `docs/reference/PROVEN-120-CONFIG.md`).
