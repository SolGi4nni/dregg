...
|---|---|---|
| (1) `sigVerify serverKey sessionKey sig` | ed25519 over Curve25519 | **NO ed25519 AIR.** The one in-AIR curve-signature verifier is **Schnorr over BabyBear^8** (`circuit/src/schnorr_curve.rs` + `circuit/src/schnorr_sig.rs`, prime-order 248-bit curve, ~124-bit). Deployed turn-auth ed25519 is verified **off-circuit** — the Ed25519↔Schnorr scale obligation is a **named terminal seam**. |
