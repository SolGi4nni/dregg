...
| 4 | Multilateral clearing conserves + atomic + respects limits | **GROUNDED** | `Dregg2/Intent/Ring.lean:118,147,156`; `Market/Fairness.lean:112` (`clearing_respects_limits`), teeth `overdebit_refused:196`, `wrongAsset_refused:203`; `crossBid_needs_market` in `metatheory/Dregg2/Intent/Kernel.lean`. Backed by real matcher `intent/src/solver.rs` (Johnson `:244`, Shapley–Scarf TTC). |
