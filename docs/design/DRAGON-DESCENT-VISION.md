...
The live game already defines the schema, simultaneous commit/reveal/resolve phases, and transition teeth (`dregg-automatafl/src/game.rs:1-31`, `dregg-automatafl/src/game.rs:47-111`). The current offering renders the board and viewer-specific sealed information (`dregg-automatafl/src/surface.rs:1-35`, `dregg-automatafl/src/surface.rs:450-579`).

⚑ 2026-07-25. This passage used to cite a Rust "reference engine" (`src/reference.rs` in `dregg-automatafl`) for the raycast, automaton step, scoring and conflicts, and a Rust proof module (`src/moves.rs`) for in-proof Poseidon-sealed moves. Both are DELETED, along with the width measurement quoted from the deleted `prove_fold.rs` test. The transition is now answered by `@[export] dregg_automatafl_rules` over `Dregg2.Games.AutomataflRules` (`dregg-automatafl/src/rules.rs`, no Rust fallback), the surface plays the stock 11×11 game, and the sealed-move leaf is Lean-emitted (`metatheory/Dregg2/Circuit/Emit/AutomataflRevealEmit.lean`). The playable surface still uses its host session seal — that gap is real; the width figure is retracted rather than restated.
