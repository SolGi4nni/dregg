...
3. **Playable and proven can be parallel lanes.** Automatafl has a real custom AIR — Lean-authored and emitted at n=11 since `f44e26e7b` deleted the hand-written Rust one, along with the `prove_fold.rs` test this bullet used to cite (`dregg-automatafl/src/lib.rs:1`, `metatheory/Dregg2/Circuit/Emit/AutomataflResolveEmit.lean`) — but its offering computes `apply_turn` on the host and commits the resulting state (`dregg-automatafl/src/surface.rs:805`, `dregg-automatafl/src/surface.rs:839`). Its own surface names the in-proof sealed move as a subsequent wiring lane (`dregg-automatafl/src/surface.rs:37`). Multiway-tug likewise advances its reference engine and commits a projection in the playable offering (`dregg-multiway-tug/src/surface.rs:345`, `dregg-multiway-tug/src/surface.rs:359`), while its fold helper constructs separate custom legs (`dregg-multiway-tug/src/fold.rs:148`).
...
Automatafl proves that a transition AIR can check `new == apply_turn(old, moves)` and expose a foldable commitment (`dregg-automatafl/src/lib.rs:1`, `dreggnet-game-board/tests/two_leg_board_window.rs`). ⚑ It proves this with a LEAN-authored AIR: the hand-authored Rust AIR and its `prove_fold.rs` test — what this line originally cited — were deleted in `f44e26e7b`, and the deployed-prover fold arm that replaced them is `#[ignore]`d and has not been run. Its offering proves a viewer-specific hidden-state surface and a real receipted game loop (`dregg-automatafl/src/surface.rs:25`, `dregg-automatafl/src/surface.rs:587`).
