...
ceremony must pin the **proven-120** FRI configuration, not the deployed `d=4` config (its proven commit posture is `deployed_wrap_commitBits`),
...
The deployed FRI commit posture at the apex (`d=4`, `|D⁰| = 2^22`) is the proven column
`FriDeployedHeightPairing.deployed_wrap_commitBits` (digit in `PROVEN-120-CONFIG.md` §3.1). The
`m`-optimized ceiling once quoted here (`PROVEN-120-CONFIG.md` §3.1) is refuted as the
...
the ceremony at `d=4` and the frozen hex KAT pins the deployed `d=4` config, and the `d=8` cutover then throws
...
- **Track B (FRI soundness) — HARD, blocking, and it dictates the sequence.** Rung D4's ceremony MUST pin
  the proven-120 `d=8` config, never the deployed `d=4` config. Therefore D4 = Phases 4-5 of
...
| deployed commit posture at d=4 (`deployed_wrap_commitBits`); proven target d=8 λ=122.60 | DERIVED (Track B) | `FriDeployedHeightPairing`; `PROVEN-120-CONFIG.md` §3.4 |
