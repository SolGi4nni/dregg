...
/// **Arm (and re-arm) the day this surface's Descent mints relics under** — fetch today's drand
/// round, BLS-verify it, publish it for [`dreggnet_catalog::DescentDayBinding::Live`]. Blocking; run it
/// at boot and on a timer, since a rolled UTC day makes the published day stale and every Descent
/// open then refuses (fail-closed, by design). The returned [`dreggnet_catalog::BeaconSource`]
/// says whether the day is today's live round or the explicitly-labeled pinned round standing in
/// for a transport outage — log it.
