...
pub use crate::bfv_lean::{LeanCiphertext, RnsPoly};
use crate::params::pick_params;

// `distributed` (t<n across processes — uses mpc_party + quorum), `quorum` (t<n VSS — pulls
// pq_share_commitment / bulletproofs), and `relin` (n-of-n relin — pulls bfv_mul) all STAY in fhegg-fhe:
// their deps would break this crate's wasm-cleanliness. fhegg-fhe declares them under its `threshold` shim,
// which re-exports this core (so `crate::threshold::{distributed,relin,quorum}` is unchanged there).
...
///
/// `coeffs` (ternary, self-sampled) is the secret. It is `pub` **only** so the native in-process ceremonies
/// (`relin`/`quorum`, which stay in fhegg-fhe) can reconstruct party secrets across the fhegg-core↔fhegg-fhe
/// crate boundary the wasm-clean extraction created (streamed-cooking-shannon Track B). A DELIBERATE, FLAGGED
/// tradeoff: it widens API SURFACE, not real exposure — this module already documents that all `n` shares are
/// co-located in one process (the SIMULATED trust boundary), so a `pub` vs private field leaks nothing an
/// attacker with process memory did not already have. ⚑ RE-ENCAPSULATE when the ceremony goes truly
/// distributed: each party is its own process holding only its own share, and no cross-crate secret access is
/// needed (the wasm party never touches another party's `KeyShare`). ⚑ ember: this is the (B)-path tradeoff to
/// review — the (A) alternative was moving the whole native subsystem into fhegg-core.
...
impl KeygenSession {
    ...
    pub fn common_random_poly(&self, params: &BfvParams) -> CommonRandomPoly {
...
pub struct ThresholdParty {
    ...
    pub keygen_session: KeygenSession,
}
...
pub fn sk_from_coeffs(coeffs: &[i64], par: &Arc<BfvParameters>) -> SecretKey {
...
pub fn aggregate_public_contributions(
    session: &KeygenSession,
