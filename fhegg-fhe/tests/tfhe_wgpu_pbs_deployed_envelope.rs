...
use fhegg_fhe::tfhe_wgpu::{
    ...
    torus_pbs_extract_keyswitch_prepared, torus_pbs_modulus_switch, TorusExternalProductParams,
    TorusKeyswitchParams, TorusMacBackend, TorusMacError, TorusWgpuAlgorithm,
};
...
fn deployed_918_by_918_dense_pbs_matches_tfhe_and_reuses_device_keys() {
    ...
    for active_steps in [64usize, 256, 512] {
        ...
        scaling.push((active_steps, prefix_cpu_time, prefix_gpu_time));
    }
    ...
    eprintln!(
        ...
        scaling
            .iter()
            .map(|(steps, cpu, gpu)| format!(
                "{steps}:cpu={:.3}ms/gpu={:.3}ms",
