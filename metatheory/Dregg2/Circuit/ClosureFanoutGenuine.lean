...
structure ClosureReadouts
  ...
  /-- ⚑ ADDED 2026-08-02, in the same commit that deleted `S_live`'s four injectivity PARAMETERS.
  The setProgram rung (`closedLogExtract_setProgram_closed`) audits the record-slot root with the
  SURFACE sponge, so it genuinely needs `compressNInjective compressN` — the one place in the cone
  where a proof READ a deleted parameter rather than merely passing it on. It sits here, beside the
  five cn-carriers it is exactly like, instead of as a binder whose implicit value no call site
  could infer once it left `S_live`'s type. This is not new floor content: the bundle already
  carried `compressNInjective` five times over and stays a `#floor_ratchet` sentinel BUNDLE. -/
...
theorem closedLogExtract_all_genuine
  ...
  | 13 => exact closedLogExtract_setProgram_closed compressN rds.hNProgram rds.rdSetProgram
