...
their five `compressNInjective` cn-carrier fields, which the deletion does not touch. (⚰ 2026-08-03:
`ClosureReadoutsLive` is DELETED — `ClosureReadoutsRealizable.not_nonempty_closureReadouts_refLH`
shows its surviving `transfer` field was false too, so it never had an inhabitant either.)

⚠ **THE "ONE PLACE A PROOF GENUINELY READ A DELETED PARAMETER" DID NOT EXIST — CORRECTED 2026-08-03.**
This paragraph recorded the setProgram rung as that place and moved its `hCompressN` to a bundle field
`ClosureReadouts.hNProgram`. `closedLogExtract_setProgram_closed` passed it to
`ClosureAll.setProgram_closedLog_sat`, whose body never mentions it, because
`RotatedKernelRefinementProgram.setProgram_descriptorRefines_sat` had already shed its own
`compressNInjective` binder on 2026-07-30. Both binders and the field are DELETED; `#floor_ratchet`
red'd on the new projection and the resolution was a discharge, not a baseline row. There was no
surviving read: all four parameters were inert, at every site.
