...
CHUNK-5 tail at `V4_0 = MV3_END`, so every pre-existing column is UNTOUCHED):
  ...
  * `cTwoCyc = eqAb · eqBa · (1 − cOccIncl 0) · (1 − cOccIncl 1) · cAnz · cBnz` — the mutual 2-cycle
    detector (both target each other's source, both unblocked inclusive, both carry). Staged through
    five degree-2 products (`cTwoCyc1…4` reusing the inclusive-occlusion NOT bits `cNOccIb`/`cNOccIa`
    already emitted by `flowThroughV2Constraints`), then its NOT bit `cNTwoCyc = 1 − cTwoCyc`.
...
/-- The 2-cycle detector staging block (6 columns): the five degree-2 products of the mutual-2-cycle
product `eqAb · eqBa · (1 − cOccIncl 0) · (1 − cOccIncl 1) · cAnz · cBnz`, then its NOT bit. -/
def cTwoCyc1 : Nat := V4_0 + 0
def cTwoCyc2 : Nat := V4_0 + 1
def cTwoCyc3 : Nat := V4_0 + 2
def cTwoCyc4 : Nat := V4_0 + 3
def cTwoCyc : Nat := V4_0 + 4
def cNTwoCyc : Nat := V4_0 + 5
/-- The FINAL corrected carry (one column per piece): `cCarryV4 = cCarryV3 · (1 − cTwoCyc)`. -/
def CV4_0 : Nat := V4_0 + 6
...
/-- (Third-wound Δ) The mutual 2-CYCLE detector `cTwoCyc = eqAb · eqBa · (1 − cOccIncl 0) ·
(1 − cOccIncl 1) · cAnz · cBnz`, staged through five degree-2 products (reusing the inclusive-occlusion
NOT bits `cNOccIb`/`cNOccIa` already emitted by `flowThroughV2Constraints`), then its NOT bit
`cNTwoCyc = 1 − cTwoCyc`. Every gate is degree ≤ 2, so the max constraint degree is UNCHANGED. -/
def twoCycConstraints : List VmConstraint2 :=
  ...
  , prodPin cTwoCyc3 cTwoCyc2 cNOccIa          -- · (1 − cOccIncl 0)
  , prodPin cTwoCyc4 cTwoCyc3 cAnz             -- · cAnz
  , prodPin cTwoCyc cTwoCyc4 cBnz              -- · cBnz
...
def cTwoCyc3 (n : Nat) : Nat := (V4_0 n) + 2
def cTwoCyc4 (n : Nat) : Nat := (V4_0 n) + 3
def cTwoCyc (n : Nat) : Nat := (V4_0 n) + 4
def cNTwoCyc (n : Nat) : Nat := (V4_0 n) + 5
def CV4_0 (n : Nat) : Nat := (V4_0 n) + 6
...
def twoCycConstraints (n : Nat) : List VmConstraint2 :=
  ...
  , prodPin (cTwoCyc3 n) (cTwoCyc2 n) (cNOccIa n)
  , prodPin (cTwoCyc4 n) (cTwoCyc3 n) (cAnz n)
  , prodPin (cTwoCyc n) (cTwoCyc4 n) (cBnz n)
...
#guard automataflResolveDesc.traceWidth == 459
...
-- (twoCyc 6 + carryV4 2 + midV4 8 = 16) = 550.
#guard automataflResolveDesc.constraints.length == 550
...
#guard (NGen.twoCycConstraints 2).length == 6
...
-- (cOccIncl 0 = cOccIncl 1 = 0 ⇒ cNOccIb = cNOccIa = 1), both carry (cAnz = cBnz = 1). The V3 surface
-- SWAPS (cCarryV3 both = 1, cMidV3[sa] = repulsor); the CHUNK-6 detector fires and KEEPS both (ruling C).
-- (1) cTwoCyc = 1. The first-stage product forces cTwoCyc1 = eq_ab·eq_ba (accepts 1 on the all-ones
--     2-cycle), and the final product gate forces cTwoCyc = cTwoCyc4·cBnz = 1; cTwoCyc = 0 is REJECTED.
...
#guard (headToExpr ((Head.lin (-1) cTwoCyc).addProd 1 [cTwoCyc4, cBnz])).eval
    (fun _ => 1) == 0                                                -- cTwoCyc = cTwoCyc4·cBnz = 1 accepted
#guard (headToExpr ((Head.lin (-1) cTwoCyc).addProd 1 [cTwoCyc4, cBnz])).eval
...
#guard (NGen.resolveConstraints 2).length == 550
...
-- descN 11: the CHUNK-6 2-CYCLE surface is n-parametric too. twoCyc = 6 gates (n-independent);
-- carryV4 = 2; midV4 = 2·n² cells. The FINAL corrected board is the trace tail.
#guard NGen.V4_0 11 == NGen.MV3_END 11
#guard (NGen.twoCycConstraints 11).length == 6
