...
/-- End of the additive tail columns — the new total width. -/
def RESR_END : Nat := RESR_LOGIC + 12

/-- Total main-trace width: the validity+occlusion prefix (`RES0`), the adjudication columns, the two
packed board commitments, and the CHUNK-1 additive tail (`INCL0 … RESR_END`). -/
def R_WIDTH : Nat := RESR_END
...
def R_WIDTH (n : Nat) : Nat := RESR_END n
...
#guard automataflResolveDesc.traceWidth == 357
...
#guard automataflResolveDesc.constraints.length == 437
...
#guard (NGen.resolveConstraints 2).length == 437
...
#guard NGen.cResolvable 11 + 1 == NGen.R_WIDTH 11
