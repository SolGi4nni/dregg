...
open Dregg2.Circuit.Emit.EffectVmEmitV2
  (graduableWide graduableWide_spec graduateV1Wide Satisfied2FaithfulWide WIDE_RANGE_WIDTHS
   rangeLookupW lookup_replaces_rangeW siteLookups_sound sitesFit_admitted mem_graduateSites)
...
theorem wideEmbeddedFee_sound_v1 (permOut : List ℤ → List ℤ) (hash : List ℤ → ℤ)
    ...
    (hemb : ∀ c ∈ (v3OfFrozenFeeWide d (sitesFit_admitted (graduableWide_spec
        (graduableWide_rotateV3WithFeePin
          (graduableWide_rotateV3FrozenAuthority hgrad))).2.1)).constraints,
  ...
  · -- the ORIGINAL fee face's v1 constraints (never the retired pins — `hclean`)
    ...
    have hmem : VmConstraint2.base c ∈ (v3OfFrozenFeeWide d (sitesFit_admitted
    ...
    have hall : siteHoldsAll hash (envAt t i)
      ...
      · intro j hj
        have hmem : VmConstraint2.lookup
            ...
            ∈ (v3OfFrozenFeeWide d (sitesFit_admitted
              (graduableWide_spec hgradr).2.1)).constraints := by
          show _
            ∈ (graduateV1Wide (rotateV3WithFeePin (rotateV3FrozenAuthority d))
                (sitesFit_admitted (graduableWide_spec hgradr).2.1)).constraints
  ...
  · -- the ORIGINAL face's range teeth, each via ITS OWN width's table (15-bit EXACT)
    ...
    have hmem : VmConstraint2.lookup (rangeLookupW r) ∈ (v3OfFrozenFeeWide d (sitesFit_admitted
        (graduableWide_spec hgradr).2.1)).constraints := by
      show _ ∈ (graduateV1Wide (rotateV3WithFeePin (rotateV3FrozenAuthority d))
        (sitesFit_admitted (graduableWide_spec hgradr).2.1)).constraints
      unfold graduateV1Wide
      simp only [List.mem_append, List.mem_map, List.mem_mapIdx]
