...
|-------------------------------------------------------|------------------------------------------------|
| C1 entry hash `entry = hash_4_to_1(cur,sym,next,0)`   | arity-4 `Poseidon2Chip` lookup (`TID_P2_NARROW`)|
...
hash_2_to_1(acc[i], entry[i])` is one arity-2 `TID_P2_NARROW` chip lookup. Together these reproduce BOTH
...
Both chip lookups ride the 18-wide `TID_P2_NARROW` bus (`poseidon2narrow`, wire 8), served by the
18-prefix of the SAME chip rows (`descriptor_ir2.rs::narrow_hist`). The 2 × 7 exposed permutation
lanes never entered any soundness conclusion here — they were pure witnessed padding — so dropping
them is byte-changing and soundness-preserving. Kill-set `[8, 22)` is exactly the trailing suffix,
...
/-! ## §1 — The trace column layout (7 base columns + copy-forward acc). -/
...
/-- Total main-trace width: 8 base columns. The 2 × 7 exposed permutation lane columns the WIDE
`TID_P2` tuples carried are GONE (E7 narrowing) — they lived only at lane positions 18..24 of the
25-wide tuples and entered no soundness conclusion. -/
def DFA_WIDTH : Nat := 8
...
def entryHashLookup : VmConstraint2 :=
  .lookup ⟨poseidon2narrow,
    chipLookupTupleNarrow [.var CURRENT, .var SYMBOL, .var NEXT, .var ZERO_LANE] ENTRY_HASH⟩
...
the five boundary pins (B1, is_first=1, seed acc, B2, B3). The chip table (`TID_P2_NARROW`) is IMPLICITLY
...
#guard (chipLookupTupleNarrow [.var CURRENT, .var SYMBOL, .var NEXT, .var ZERO_LANE] ENTRY_HASH).length
         == CHIP_RATE + 1 + 1
#guard (chipLookupTupleNarrow [.var ACC, .var ENTRY_HASH] RUNNING_HASH).length
         == CHIP_RATE + 1 + 1
-- The narrowing dropped exactly 2 × (CHIP_OUT_LANES − 1) = 14 main columns for the same two forced
-- digest equations.
#guard DFA_WIDTH + 2 * (CHIP_OUT_LANES - 1) == 22
