...
theorem holdsAt_of_dc2 {hash : List ℤ → ℤ} {tf : TraceFamily} {env : VmRowEnv} {f l : Bool}
  ...
  | chalGate w   => exact (Dregg2.Circuit.DecideSatisfied2.decideChal_iff env l w).mp h
...
The abstract pushdown machine (`MRow`/`MStep`/`MFinal`/`MRun`/`rulesOf`) and the assembly
`mrun_imp_replay` — the design's named hard multi-row induction, stack as the invariant — carry NO
circuit-specific content, so they live BELOW this file in `Dregg2.Crypto.AbstractMachine` (extracted
verbatim from the §6/§6.1 that used to be inline here; that extraction is what removed the
Crypto←Circuit import `ReplayAsCert` needed to state its subsumption against the real defs).

The `export` below re-publishes every machine name into THIS namespace, so every downstream
statement (`DyckStackReplay`'s decode glue, `ReplayAsCert.subsumption_stmt_eq`) resolves
`Dregg2.Circuit.Emit.DyckStackRefine.mrun_imp_replay` etc. to the SAME declarations as before —
same names, same types, one home lower in the tower. §6.1's non-vacuity witnesses
(`bRow0`/`bracketsRest`/`bracketsRows_run`/`rulesOf_brackets`/`abs_brackets_accepts`/
`abs_brackets_in_language` — `build_brackets_witness`'s action sequence, the SAME machine `witTrace`
(§5) encodes in columns) moved with it. -/

export Dregg2.Crypto.AbstractMachine
  (Act Act.rule Act.term Act.done MRow MRow.mk MStep MFinal
   MRun MRun.last MRun.step rulesOf mrun_imp_replay
   bRow0 bracketsRest bracketsRows_run rulesOf_brackets
   abs_brackets_accepts abs_brackets_in_language)
