...
keystone chain (the wide runnable schedule (`cellDestroyVmDescriptorWide` rides `wideHashSites`)), so the site's commitment equivocation at the SAMPLED oracle
...
/-- **⚡⚡ THE CELL-DESTROY FULL-STATE BINDING, DISCHARGED** — the ROM successor of
`cellDestroy_runnable_full_commit_binds_or_collides`: every query-bounded forger that equivocates
the whole 17-field nested wide commitment has NEGLIGIBLE advantage (`wideRow_binds_rom`). -/
theorem cellDestroy_runnable_full_commit_binds_rom (D : SpongeKeyed) (tagDec : DecidableEq D.Tag)
...
/-- **⚡ THE CELL-DESTROY SIDE-TABLE TOOTH, DISCHARGED** — the ROM successor of
`cellDestroy_rejects_root_tamper_or_collides` (`wide_root_tamper_rom`). -/
theorem cellDestroy_rejects_root_tamper_rom (D : SpongeKeyed) (tagDec : DecidableEq D.Tag)
    (Q : ℕ → ℕ) (hQ : PolyBounded (fun l => ((Q l : ℝ) * (Q l : ℝ) + 1)))
...
#assert_axioms cellDestroy_runnable_full_commit_binds_rom
#assert_axioms cellDestroy_rejects_root_tamper_rom
