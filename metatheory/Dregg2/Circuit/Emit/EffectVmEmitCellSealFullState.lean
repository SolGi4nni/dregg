...
keystone chain (the wide runnable schedule (`cellSealVmDescriptorWide` rides `wideHashSites`)), so the site's commitment equivocation at the SAMPLED oracle
...
/-- **⚡⚡ THE CELL-SEAL FULL-STATE BINDING, DISCHARGED** — the ROM successor of
`cellSeal_runnable_full_commit_binds_or_collides`: every query-bounded forger that equivocates
the whole 17-field nested wide commitment has NEGLIGIBLE advantage (`wideRow_binds_rom`). -/
theorem cellSeal_runnable_full_commit_binds_rom (D : SpongeKeyed) (tagDec : DecidableEq D.Tag)
...
/-- **⚡ THE CELL-SEAL SIDE-TABLE TOOTH, DISCHARGED** — the ROM successor of
`cellSeal_rejects_root_tamper_or_collides` (`wide_root_tamper_rom`). -/
theorem cellSeal_rejects_root_tamper_rom (D : SpongeKeyed) (tagDec : DecidableEq D.Tag)
    (Q : ℕ → ℕ) (hQ : PolyBounded (fun l => ((Q l : ℝ) * (Q l : ℝ) + 1)))
...
#assert_axioms cellSeal_runnable_full_commit_binds_rom
#assert_axioms cellSeal_rejects_root_tamper_rom
