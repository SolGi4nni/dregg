...
theorem emitEventVm_commit_binds_keyed (Q : ℕ → ℕ)
    ...
    (hA : NarrowBreakEff emitEventVmDescriptor.hashSites Q A) :
    Negl (gameAdv (narrowBreakGame emitEventVmDescriptor.hashSites) A) :=
  narrowStateCommit_binds emitEventVmDescriptor.hashSites rfl Q hQ A hA
