...
def bbRange64 (c : Ctx) (x : BB) : Ctx :=
  ...
  c.emit (.rc0 x.v z1 z2 (gc 0) (gc 1) (gc 2) (gc 3)
...
def emitPerm (xs : List Nat) : List BB × Ctx :=
  let (inp, c) := inputLanes Ctx.empty xs
  permGen c inp
...
**Rows: 2,668 emitted against 2,602 measured in o1js — +66, or +2.5%.**

| | Generic | RangeCheck0 | Lookup | total |
|---|---:|---:|---:|---:|
| o1js 2.15.0, `Provable.constraintSystem` | 1,623 | 701 | 278 | **2,602** |
| this generator | 1,542 | 1,126 | 0 | **2,668** |
...
**+2.5% is the honest reading and it is not a match.** The number that matters is that a GENERATED
circuit prices within a few percent of a hand-tuned one while computing the KAT: the compiler is
not paying a structural penalty for being a compiler. At 2,668 rows the Pickles arithmetic moves
...
#guard permRowCount = 2668

/-! The emitted gate mix, pinned: generic rows and range-check rows, nothing else. -/
#guard permHistogram.filter (fun x => x.2 != 0)
  = [(KGateType.generic, 1542), (KGateType.rangeCheck0, 1126)]
