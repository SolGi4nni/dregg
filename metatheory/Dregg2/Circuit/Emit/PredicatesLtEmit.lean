...
theorem predicateLtAir_mainRailOk : predicateLtAir.mainRailOk = true := by decide
...
theorem predicateLtAir_pinsTied : predicateLtAir.pinsTied = true := by decide
...
def predicateLtTiedAir : Dregg2.Circuit.Emit.EffectLower.TiedAir where
  air  := predicateLtAir
  ok   := predicateLtAir_mainRailOk
  tied := predicateLtAir_pinsTied
