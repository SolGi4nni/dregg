...
/-- The forged factory-leaf predicate over `oneLaneDemoEngine` (the only verifying sub-proof commits
to `123`).
...
`demoFLS_sat` is the companion that keeps this predicate from being false everywhere. -/
def demoFLS : ℤ → ℤ → Prop :=
  fun _leafVk leafCommit =>
    ∃ q : Bool, oneLaneDemoEngine.verify q = true ∧ oneLaneDemoEngine.piCommit q = [leafCommit]

theorem demoFloor : FactoryLeafFriFloor oneLaneDemoEngine demoFLS :=
  fun _leafVk _leafCommit h => h

/-- **`demoFLS_sat` — THE PREDICATE IS SATISFIED AT THE HONEST VALUE, so the pole below refuses a
VALUE rather than everything.** Without it `forged_childvk_unsat_demo` is equally consistent with a
...
def forgedFold : FactoryFold oneLaneDemoEngine := { leafVk := 0, leafCommit := 999, cv := 999 }
...
theorem forged_childvk_unsat_demo : ¬ SatFactoryFold oneLaneDemoEngine demoFLS forgedFold := by
  ...
  -- VALUE (paired with `demoFLS_sat`, which fires at `123`), not about the arity.
