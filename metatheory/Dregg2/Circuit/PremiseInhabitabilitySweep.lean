...
| R8 | ⛑ **NEW 2026-07-30** — the corrected bundles' ADDED conjunct `¬ OpeningColl sponge idx topen.constraintEval vCommitted siblings` (`ApexOodLaneRepair.FriLdtExtractCons`, `FriFsDecodedOodRepair.DecodedLdtLinkCons`, `OodSingletonRepair.DecodedLdtLinkExtCons`, and the `…NoOodShape` twins; already on `FriLdtExtractDeployed.FriLdtExtractV3Cons` since 2026-07-28) | see left | **DISCHARGEABLE, and it is the honest replacement for a REFUTED hypothesis** — the conjunct does not tighten inhabitability in the way R1 does | `OodCommitmentBinding.openingColl_self_false` discharges it with NO hypothesis on the sponge whenever the run publishes one value per committed slot, and `ApexOodLaneRepair.consResidual_iff_opening_functional` shows it is EQUIVALENT, at two values recomputing to the same root, to those values being equal — i.e. it asks only that the run did not equivocate at its OWN extracted opening. It replaced `Poseidon2SpongeCR sponge` binders on 42 declarations; that hypothesis is PROVED FALSE at deployed BabyBear parameters (`poseidon2SpongeCR_false_babyBear`), so the trade is an unsatisfiable global hypothesis for a per-run dischargeable one. ⚠ NOT claimed: that the bundles are inhabited at the deployed `opaque` arguments — R1 is still the binding obstruction and this conjunct does not touch it. |
