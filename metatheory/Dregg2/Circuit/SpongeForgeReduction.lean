...
EFFICIENT ADVERSARIES. The cure is the standard one:
  ...
  * the security conclusion is `Negl` under the NAMED floor
    `DomainSeparatedCREffRegrounded.DeployedSpongeCRHard D Eff`
    = `FloorGames.HashCRHardQuant (spongeFamily D) Eff`,
...
The floor is FALSE at `Eff := ⊤` and vacuous at `Eff := ⊥`; both are proved below. What the reduction
buys is that the residual is ONE named parameter with both poles proved and its `IsPolyTime`
instantiation's efficiency obligation DISCHARGED — in place of a hypothesis the deployed hash REFUTES.
It does not make the deployed BabyBear sponge collision-resistant; nothing can.
...
keyed-from-unkeyed model. `DeployedSponge` bundles exactly that, and `spongeFamily` lifts it to a
`HashFloorHonesty.KeyedHashFamily`, so the floor below is `FloorGames.HashCRHardQuant` at the family
whose instance AT THE DEPLOYED TAG **is** the function the prover computes (`deployed_is_family_instance`
— a definitional equality, no idealization).

⚑ This module deliberately depends ONLY on `FloorGames` + `HashFloorHonesty` + `CostAdversary`/
`CostTactics` — the grounded floor. It does not route through the `Poseidon2KeyedBridge` /
`DomainSeparatedCREffRegrounded` presentation of the same object, so the reduction cannot be broken by
churn in the consumer-re-export layers. -/

/-- **THE DEPLOYED SPONGE with its domain-separation structure.** `sponge` is the deployed fixed
`List ℤ → ℤ`; `Tag` is the finite inhabited domain-separation tag space (the effective key the CR game
samples); `tagCode` is how a tag is absorbed as a prefix; `deployedTag` is the tag the deployment
actually computes at this use-site. -/
structure DeployedSponge where
  /-- The deployed unkeyed Poseidon2 sponge. -/
  sponge : List ℤ → ℤ
  /-- The domain-separation tag space (the effective key). -/
  Tag : Type
  /-- The tag space is finite (the collision game samples a uniform tag). -/
...
/-- The deployed sponge domain-separated by a SPECIFIC tag: `xs ↦ sponge (tagCode t ++ xs)`. -/
def DeployedSponge.hashAt (D : DeployedSponge) (t : D.Tag) (xs : List ℤ) : ℤ :=
  D.sponge (D.tagCode t ++ xs)

/-- **THE DEPLOYED FIXED FUNCTION the prover actually computes** — the sponge at the deployed tag. This
is the real object every game below is about. -/
def DeployedSponge.deployedHash (D : DeployedSponge) (xs : List ℤ) : ℤ :=
  D.hashAt D.deployedTag xs

/-- The deployed sponge lifted to a `KeyedHashFamily`, keyed by its domain-separation tag. -/
def spongeFamily (D : DeployedSponge) : KeyedHashFamily where
  Key := fun _ => D.Tag
  Input := List ℤ
  Out := ℤ
...
/-- **FAITHFULNESS.** The DEPLOYED fixed unkeyed function IS the keyed family's instance at the deployed
tag — a definitional equality. So the floor below is an assumption about the very function the prover
computes, not about an abstraction of it. -/
theorem deployed_is_family_instance (D : DeployedSponge) (n : ℕ) :
    D.deployedHash = (spongeFamily D).H n D.deployedTag := rfl

/-- **⚑ THE NAMED FLOOR.** Every adversary IN THE CLASS `Eff` finds a collision of the deployed
domain-separated sponge, under a uniformly sampled tag, only with negligible probability. This is
`FloorGames.HashCRHardQuant` at the deployed family — the honest computational floor, NOT injectivity
(false) and NOT `∃ collision` (a solution, available by pigeonhole). Its honesty is exactly `Eff`'s, and
both poles are proved in §5. -/
def DeployedSpongeCRHard (D : DeployedSponge)
    (Eff : Adversary (hashGame (spongeFamily D)) → Prop) : Prop :=
  HashCRHardQuant (spongeFamily D) Eff
...
def codeForgeGame (D : DeployedSponge) (A : Type) (code : A → List ℤ) : Game where
  Inst := fun _ => D.Tag
  Ans := fun _ => A × A
...
`hashGame (spongeFamily D)`'s win. This is the COMMON TARGET every multi-absorb site's
...
abbrev spongeCollGame (D : DeployedSponge) : Game := codeForgeGame D (List ℤ) id

/-- A `spongeCollGame` win IS a collision of the deployed domain-separated sponge at the sampled tag. -/
theorem spongeCollGame_wins_iff (D : DeployedSponge) (l : ℕ) (t : D.Tag) (p : List ℤ × List ℤ) :
...
theorem codeForgeGame_wins_iff (D : DeployedSponge) (A : Type) (code : A → List ℤ)
    (l : ℕ) (t : D.Tag) (p : A × A) :
...
theorem deployed_forgery_is_break (D : DeployedSponge) (A : Type) (code : A → List ℤ)
...
def codeForgeToFinder (D : DeployedSponge) (A : Type) (code : A → List ℤ)
...
`sponge (tagCode t ++ ·)`. Definitional, because `spongeFamily`'s instance at `t` IS
`D.hashAt t`. -/
theorem code_wins_imp (D : DeployedSponge) (A : Type) (code : A → List ℤ)
    (adv : Adversary (codeForgeGame D A code)) (l : ℕ) (t : D.Tag)
    (hwin : (codeForgeGame D A code).wins l t (adv.run l t)) :
    (hashGame (spongeFamily D)).wins l t ((codeForgeToFinder D A code adv).run l t) :=
  hwin
...
theorem code_adv_le (D : DeployedSponge) (A : Type) (code : A → List ℤ)
    (adv : Adversary (codeForgeGame D A code)) (l : ℕ) :
    gameAdv (codeForgeGame D A code) adv l
      ≤ gameAdv (hashGame (spongeFamily D)) (codeForgeToFinder D A code adv) l := by
...
Under `DeployedSpongeCRHard D Eff` (the deployed domain-separated sponge's collision floor at the class
...
theorem codeForge_advantage_bound (D : DeployedSponge) (A : Type) (code : A → List ℤ)
    (Eff : Adversary (hashGame (spongeFamily D)) → Prop)
    (adv : Adversary (codeForgeGame D A code))
    (hEff : Eff (codeForgeToFinder D A code adv))
    (hCR : DeployedSpongeCRHard D Eff) :
...
def codeAnsSize (D : DeployedSponge) (A : Type) (code : A → List ℤ) :
...
def hashAnsSize (D : DeployedSponge) : AnsSize (hashGame (spongeFamily D)) :=
...
theorem codeForge_out_le (D : DeployedSponge) (A : Type) (code : A → List ℤ)
    (l : ℕ) (t : D.Tag) (p : A × A) :
...
theorem codeForge_binds_from_polyTime (D : DeployedSponge) (A : Type) (code : A → List ℤ)
    (adv : Adversary (codeForgeGame D A code))
    (hA : IsPolyTime (codeAnsSize D A code) adv) (cw bw : ℕ)
    (hCR : DeployedSpongeCRHard D (IsPolyTime (hashAnsSize D))) :
...
theorem forge_floor_top_false_babyBear (D : DeployedSponge)
    (hb : ∀ xs, 0 ≤ D.sponge xs ∧ D.sponge xs < (2013265921 : ℤ)) :
    ¬ DeployedSpongeCRHard D (fun _ => True) :=
  effFloor_top_false_babyBear D hb

/-- **THE ⊥ POLE — vacuous.** Recorded so the floor's satisfiability cannot be mistaken for evidence. -/
theorem forge_floor_bot_vacuous (D : DeployedSponge) :
    DeployedSpongeCRHard D (fun _ => False) :=
...
theorem hashFloor_isPolyTime_inhabited (D : DeployedSponge) :
...
example (D : DeployedSponge) (A : Type) (code : A → List ℤ)
    (Eff : Adversary (hashGame (spongeFamily D)) → Prop)
    (adv : Adversary (codeForgeGame D A code))
    (B : Adversary (hashGame (spongeFamily D))) (hB : Eff B)
    (hCR : DeployedSpongeCRHard D Eff) : True := by
  fail_if_success
...
theorem the_reduced_forge_bound_fires (D : DeployedSponge) (A : Type) (code : A → List ℤ)
    (Eff : Adversary (hashGame (spongeFamily D)) → Prop)
    (adv : Adversary (codeForgeGame D A code))
    (hEff : Eff (codeForgeToFinder D A code adv))
    (hCR : DeployedSpongeCRHard D Eff) :
...
theorem injective_code_forgery_is_break (D : DeployedSponge) (A : Type) (code : A → List ℤ)
...
theorem digest_binds_or_collides (D : DeployedSponge) (A : Type) (code : A → List ℤ)
