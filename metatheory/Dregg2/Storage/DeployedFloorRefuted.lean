...
`Dregg2/Storage/Deployed.lean:32-33` routes every limb through `x.toNat.toUInt64`, and `Int.toNat`
clamps every negative to `0`. So `poseidon2Hash [-1] = poseidon2Hash [0]` holds **by `rfl`** — the
collision needs no unfolding of the opaque `p2compress` at all.

More importantly this is **not repairable by fixing the encoder**. `poseidon2Hash : List ℤ → ℤ`
factors through `UInt64`, so its image is *finite* while its domain is *infinite*. Exact injectivity
is therefore unsatisfiable at **any** encoding. The floor is mis-specified, not mis-implemented.
...
/-- The collision, by `rfl`: `Int.toNat` clamps `-1` and `0` to the same `UInt64` limb, so the
deployed hash cannot distinguish them. No property of `p2compress` is used. -/
theorem deployed_collides : poseidon2Hash [(-1 : Int)] = poseidon2Hash [(0 : Int)] := rfl
...
theorem deployed_floor_false : ¬ Poseidon2SpongeCR poseidon2Hash := by
  intro h
  have := h [(-1 : Int)] [(0 : Int)] deployed_collides
  simp at this
