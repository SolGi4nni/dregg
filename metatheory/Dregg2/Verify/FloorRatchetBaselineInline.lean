...
/-- ⚑ EMPTY since 2026-08-02. Every name that was here is now a KEYED row
(`name ⊣ floor+floor+…`) in `FloorRatchetBaseline.lean`, folded in by the one-pass
`#floor_ratchet_emit` this file's header describes. Left in place they would have been LEGACY
name-only rows that no emit can ever reach — nothing rewrites this file — i.e. a blanket
exemption over the whole refuted-floor set, for each of those declarations, forever.

Do not refill it. A new inline-spelled carrier is grandfathered the same way as any other:
paste the KEYED row the failing gate prints into `FloorRatchetBaseline.manual`. -/
