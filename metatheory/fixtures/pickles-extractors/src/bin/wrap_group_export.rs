...
/// A Pallas point's affine coordinates as DECIMAL strings — the SAME extraction `pallas_j` uses
/// (`AffineRepr::xy` then `dec_of_le(into_bigint().to_bytes_le())`), so the generated Lean SRS
/// fixture is in the same encoding as every other number in this campaign.
fn pallas_dec(p: &Pallas) -> (String, String) {
    match p.xy() {
        Some((x, y)) => (dfp(&x), dfp(&y)),
        None => panic!("the point at infinity has no affine coordinates"),
    }
}

// ------------------------------------------------------------------ rung 5h fixture emission

/// Points per generated Lean `def`: 64 blocks of 512 rather than one 32768-element list literal.
const SRS_BLOCK: usize = 512;

/// Writes `out/MinaWrapSrsG.lean` — the 2^15 Wrap SRS generators as Lean `List Pt` blocks —
/// `out/MinaWrapSgParts.lean` — the 32 chunk partials and `sg` — and `out/srs_fold_gold.json` —
/// the 15 IPA challenges, their inverses, `sg`, the fold gold and the partials.
/// None of it is committed; where it lands is the operator's call.
...
fn emit_srs_fixture(
    ...
    sg: &Pallas,
    fold_gold: &Pallas,
    chunk_partials: &[Pallas],
    chunk_size: usize,
) {
    use std::io::Write as _;

    for (i, p) in srs_g.iter().enumerate() {
        assert!(p.xy().is_some(), "srs.g[{i}] is the point at infinity");
    }
    assert_eq!(
        srs_g.len() % SRS_BLOCK,
        0,
        "srs.g does not split into whole {SRS_BLOCK}-point blocks"
    );
    let nblocks = srs_g.len() / SRS_BLOCK;

    let out = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("out");
    std::fs::create_dir_all(&out).expect("create out/");

    // ---- the Lean module -------------------------------------------------
    let lean_path = out.join("MinaWrapSrsG.lean");
    let mut w = std::io::BufWriter::new(
        std::fs::File::create(&lean_path).expect("create MinaWrapSrsG.lean"),
    );
    let mut lines: usize = 0;
    let wl = |w: &mut std::io::BufWriter<std::fs::File>, s: &str, lines: &mut usize| {
        writeln!(w, "{s}").expect("write");
        *lines += 1;
    };
    wl(&mut w, "/-! GENERATED by `wrap_group_export.rs` — the devnet blockchain Wrap SRS generators `srs.g`,", &mut lines);
    wl(
        &mut w,
        &format!(
            "{} Pallas points in projective form `(x, y, 1)`, decimal. DO NOT EDIT. -/",
            srs_g.len()
        ),
        &mut lines,
    );
    wl(
        &mut w,
        "namespace Dregg2.Circuit.Emit.MinaWrapSrsG",
        &mut lines,
    );
    wl(&mut w, "abbrev Pt := Nat × Nat × Nat", &mut lines);
    for b in 0..nblocks {
        wl(&mut w, &format!("def blk{b} : List Pt := ["), &mut lines);
        for k in 0..SRS_BLOCK {
            let (x, y) = pallas_dec(&srs_g[b * SRS_BLOCK + k]);
            let sep = if k + 1 == SRS_BLOCK { "]" } else { "," };
            wl(&mut w, &format!("  ({x}, {y}, 1){sep}"), &mut lines);
        }
    }
    let names: Vec<String> = (0..nblocks).map(|b| format!("blk{b}")).collect();
    wl(
        &mut w,
        &format!("def SRS_G : List Pt := {}", names.join(" ++ ")),
        &mut lines,
    );
    wl(&mut w, "end Dregg2.Circuit.Emit.MinaWrapSrsG", &mut lines);
    w.flush().expect("flush MinaWrapSrsG.lean");
    drop(w);

    // ---- the chunk-partials Lean module ----------------------------------
    // `SG` is emitted from `sg` itself; it is asserted equal to GT9's fold gold so the two
    // generated modules cannot disagree about which point they are aimed at.
    assert_eq!(
        sg, fold_gold,
        "sg and the GT9 fold gold disagree — the fixtures would be inconsistent"
    );
    for (k, p) in chunk_partials.iter().enumerate() {
        assert!(
            p.xy().is_some(),
            "chunk partial {k} is the point at infinity — Lean `Pt` has no (0,1,0) arm here"
        );
    }
    let parts_path = out.join("MinaWrapSgParts.lean");
    let mut parts_src = String::new();
    parts_src.push_str("/-! GENERATED by `wrap_group_export.rs`. DO NOT EDIT. -/\n");
    parts_src.push_str("namespace Dregg2.Circuit.Emit.MinaWrapSgParts\n");
    parts_src.push_str("abbrev Pt := Nat × Nat × Nat\n");
    parts_src.push_str(&format!(
        "/-- arkworks' `msm_bigint` over chunk k = points/scalars [{chunk_size}*k, {chunk_size}*k+{chunk_size}). -/\n"
    ));
    parts_src.push_str("def PARTS : List Pt := [\n");
    for (k, p) in chunk_partials.iter().enumerate() {
        let (x, y) = pallas_dec(p);
        let sep = if k + 1 == chunk_partials.len() {
            "]"
        } else {
            ","
        };
        parts_src.push_str(&format!("  ({x}, {y}, 1){sep}\n"));
    }
    parts_src.push_str("/-- the block's own `opening.sg`. -/\n");
    let (sx, sy) = pallas_dec(sg);
    parts_src.push_str(&format!("def SG : Pt := ({sx}, {sy}, 1)\n"));
    parts_src.push_str("end Dregg2.Circuit.Emit.MinaWrapSgParts\n");
    std::fs::write(&parts_path, &parts_src).expect("write MinaWrapSgParts.lean");
    ...
    let json_path = out.join("srs_fold_gold.json");
    let mut j = String::new();
    j.push_str("{\n");
    j.push_str(&format!("  \"srs_g_len\": {},\n", srs_g.len()));
    j.push_str(&format!("  \"chals\": [{}],\n", js(chal)));
    j.push_str(&format!("  \"chal_invs\": [{}],\n", js(chal_inv)));
    j.push_str(&format!("  \"sg\": {},\n", pallas_j(sg)));
    j.push_str(&format!("  \"fold_gold\": {},\n", pallas_j(fold_gold)));
    j.push_str(&format!("  \"chunk_size\": {chunk_size},\n"));
    j.push_str(&format!("  \"n_chunks\": {},\n", chunk_partials.len()));
    j.push_str(&format!(
        "  \"chunk_partials_{chunk_size}\": [{}]\n",
        chunk_partials
            .iter()
            .map(|p| {
                let (x, y) = pallas_dec(p);
    ...
    j.push_str("}\n");
    std::fs::write(&json_path, &j).expect("write srs_fold_gold.json");

    let bytes = std::fs::metadata(&lean_path)
        .expect("stat lean fixture")
        .len();
    eprintln!(
        "[5h] wrote {} : {} points = {} blocks x {}, {} lines, {} bytes",
        lean_path.display(),
        srs_g.len(),
        nblocks,
        SRS_BLOCK,
    ...
    eprintln!(
        "[5h] wrote {} : {} chunk partials + SG, {} bytes",
        parts_path.display(),
        chunk_partials.len(),
        parts_src.as_bytes().len()
    );
    eprintln!(
        "[5h] wrote {} : {} bytes",
        json_path.display(),
...
fn main() {
    ...
    // -- GROUND TRUTH 9: the generator FOLD recipe -------------------------------------------
    // GT8's object is a 2^15-term MSM, which is not a shape a Lean kernel can run. The SAME point
    // is reachable by 15 rounds of halving the generator vector — `g <- g_lo + c_j * g_hi`, FIRST
    // challenge FIRST — i.e. 2^15 - 1 group additions and 2^15 - 1 scalar-muls, no MSM at all.
    // `combine_one_endo` is deliberately NOT called: what is pinned here is the plain-arithmetic
    // recipe, because that is the object Lean will reproduce.
    let srs_g: Vec<Pallas> = vi.srs().g.clone();
    let fold_gold: Pallas = {
        let chals: &[Fq] = &ipa_chals.chal;
        let mut g: Vec<Pallas> = srs_g.clone();
        assert_eq!(g.len(), 1usize << chals.len());
        eprintln!(
            "[gt9] srs.g.len() = {} chals.len() = {} 2^chals.len() = {}",
            srs_g.len(),
            chals.len(),
            1usize << chals.len()
        );
        let mut round_lens: Vec<usize> = vec![g.len()];
        for c in chals.iter() {
            // round j uses chals[j], FIRST challenge FIRST
            let n = g.len() / 2;
            let next: Vec<Pallas> = {
                let (lo, hi) = g.split_at(n);
                lo.iter()
                    .zip(hi.iter())
                    .map(|(l, h)| (l.into_group() + *h * *c).into_affine())
                    .collect()
            };
            g = next;
            round_lens.push(g.len());
        }
        assert_eq!(g.len(), 1);
        eprintln!(
            "[gt9] per-round |g| : {}",
        ...
        eprintln!(
            "[gt9] fold(srs.g, chal) == <s,srs.g> : {}",
            g[0] == sg_from_srs
        );
        eprintln!(
            "[gt9] fold(srs.g, chal) == opening.sg : {}",
            g[0] == pp.proof.sg
        );
        assert_eq!(
            g[0], sg_from_srs,
            "the FOLD recipe does not reproduce <s, srs.g>"
        );
        assert_eq!(g[0], pp.proof.sg, "the FOLD recipe does not reproduce sg");
        let (gx, gy) = pallas_dec(&g[0]);
        eprintln!("[gt9] fold_gold.x = {gx}");
        eprintln!("[gt9] fold_gold.y = {gy}");
        g[0]
    };

    // -- GT9's ANTI-VACUITY / ORIENTATION CONTROLS -------------------------------------------
    // Three conventions the recipe could have been wrong about. Each must come out FALSE; if any
    // is TRUE the identity is INSENSITIVE to that convention and GT9 pins nothing about it.
    {
        // The same plain fold, parameterised by the convention under test.
        let fold = |cs: &[Fq], swap_halves: bool| -> Pallas {
            let mut g: Vec<Pallas> = srs_g.clone();
            for c in cs.iter() {
                let n = g.len() / 2;
                let next: Vec<Pallas> = {
                    ...
                    lo.iter()
                        ...
                        .map(|(l, h)| {
                            if swap_halves {
                                (h.into_group() + *l * *c).into_affine()
                            } else {
                                (l.into_group() + *h * *c).into_affine()
                            }
                        })
                        .collect()
                };
        ...
        let reversed: Vec<Fq> = ipa_chals.chal.iter().rev().copied().collect();
        let ctl_a = fold(&reversed, false);
        let ctl_b = fold(&ipa_chals.chal, true);
        let ctl_c = fold(&ipa_chals.chal_inv, false);
        eprintln!(
            "[gt9-ctl-a] REVERSED challenge order (chal[k-1] consumed first) == sg : {}",
            ctl_a == pp.proof.sg
        );
        eprintln!(
            "[gt9-ctl-b] SWAPPED halves (g_hi + c*g_lo instead of g_lo + c*g_hi) == sg : {}",
            ctl_b == pp.proof.sg
        );
        eprintln!(
            "[gt9-ctl-c] challenge INVERSES (chal_inv) instead of chal == sg : {}",
            ctl_c == pp.proof.sg
        );
        assert_ne!(
            ctl_a, pp.proof.sg,
            "the fold is INSENSITIVE to challenge ORDER — GT9 pins no orientation"
        );
        assert_ne!(
            ctl_b, pp.proof.sg,
            "the fold is INSENSITIVE to which half carries the challenge — GT9 pins no orientation"
        );
        assert_ne!(
            ctl_c, pp.proof.sg,
            "the fold is INSENSITIVE to chal vs chal_inv — GT9 pins no orientation"
        );
    }

    // -- GROUND TRUTH 10: the CHUNK PARTIALS -------------------------------------------------
    // The Lean kernel cannot discharge a 2^15-term MSM in ONE `decide` — elaborator memory tracks
    // the largest single `decide`. So the in-kernel discharge is split into 32 contiguous chunks
    // of 1024, each its own theorem, and each chunk's partial sum has to be a pinned value that
    // arkworks produced. The only thing that makes the split sound is that the partials RE-SUM to
    // `sg`, which is asserted here and refuted under a one-point perturbation.
    const NCHUNK: usize = 32;
    const CSZ: usize = 1024;
    let chunk_partials: Vec<Pallas> = {
        let s = poly_commitment::commitment::b_poly_coefficients(&ipa_chals.chal);
        assert_eq!(s.len(), srs_g.len());
        assert_eq!(
            NCHUNK * CSZ,
            srs_g.len(),
            "the chunking does not cover srs.g"
        );
        let mut parts: Vec<Pallas> = Vec::new();
        for k in 0..NCHUNK {
            use ark_ec::VariableBaseMSM;
            let lo = k * CSZ;
            let bigs: Vec<_> = s[lo..lo + CSZ].iter().map(|x| x.into_bigint()).collect();
            parts.push(
                <Pallas as AffineRepr>::Group::msm_bigint(&srs_g[lo..lo + CSZ], &bigs)
                    .into_affine(),
            );
        }
        assert_eq!(parts.len(), NCHUNK);
        let resum = |ps: &[Pallas]| -> Pallas {
            ps.iter()
                .fold(<Pallas as AffineRepr>::Group::zero(), |a, p| {
                    a + p.into_group()
                })
                .into_affine()
        };
        let total = resum(&parts);
        eprintln!(
            "[gt10] {NCHUNK} x {CSZ} chunk partials re-sum to sg : {}",
            total == pp.proof.sg
        );
        assert_eq!(
            total, pp.proof.sg,
            "the chunk partials do NOT re-sum to sg — the split is unsound"
        );
        // anti-vacuity: perturbing ONE partial must break the re-sum
        let mut bent = parts.clone();
        bent[7] = (bent[7] + Pallas::generator()).into_affine();
        let bent_total = resum(&bent);
        eprintln!(
            "[gt10] re-sum with partial[7] displaced by +G (negative control) == sg : {}",
            bent_total == pp.proof.sg
        );
        assert_ne!(
            bent_total, pp.proof.sg,
            "the re-sum survived a displaced partial — GT10 is vacuous"
        );
        // the Lean projective encoding writes O as (0,1,0); say whether it has to.
        let inf = parts.iter().filter(|p| p.xy().is_none()).count();
        eprintln!("[gt10] chunk partials that are the point at infinity : {inf} of {NCHUNK}");
        parts
    };

    // -- rung 5h's fixture: the 2^15 generators, for the Lean side ---------------------------
    emit_srs_fixture(
        &srs_g,
        &ipa_chals.chal,
        &ipa_chals.chal_inv,
