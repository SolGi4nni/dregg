/-
  ...
  ASSURANCE: `#print axioms` ⊆ {propext, Quot.sound, Classical.choice},
  /`ofReduceBool`. This is Stack L (the Lean model of Pancake);
  ...
  [CLOSED 2026-07-17 — Pancake/NatToDecFull.lean: `natToDecProg_sem` proves the
  do-while digit loop (`digitBody; while (n) digitBody`) renders the WHOLE
  `natToDec m` byte string at `[p0 - L, p0)`, exact `natFuel`/`digitFuel` clock
  accounting, descending `StoreByte` framing included.]
-/
