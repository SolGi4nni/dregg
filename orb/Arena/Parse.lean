...
theorem sliceArr_toArray (bs : Bytes) (s : Span) :
  ...
  rw [Array.toList_extract, List.toList_toArray, List.extract_eq_take_drop]
  congr 1
  omega
...
theorem canonNameEntryAcc_toArray (input : Bytes) (sacc : Array UInt8) (sp : Span) :
  ...
  · simp [h, Array.toList_append, Array.length_toList]
  · simp [h]
...
@[csimp] theorem parseHeaders_eq_fast : @parseHeaders = @parseHeadersFast := by
  ...
  rw [parseHeadersFast, parseHeadersAcc_toArray, List.toList_toArray]
