...
theorem findCrlf_append_no_cr : ∀ (pre rest : Bytes), (∀ b ∈ pre, b ≠ CR) →
  ...
  | cons a pre' ih =>
    intro rest h
    have ha : a ≠ CR := h a (List.mem_cons_self)
...
theorem parseHeader_line (pre rest : Bytes) (hpre : ∀ b ∈ pre, b ≠ CR) :
  ...
  have htake : (pre ++ CR :: LF :: rest).take pre.length = pre := List.take_left
...
/-- Tail-recursive engine behind `decodeStream`, so the framing decode runs in
O(1) native stack. `fuel` bounds the frame count (each data chunk consumes ≥ 1
octet, so `buf.length` steps always suffice); `accRev` holds the decoded chunk
payloads seen so far in reverse and `accC` their consumed-octet total. The
self-call is in tail position, so the compiler emits a loop and a body with
unboundedly many chunks no longer grows the stack. -/
def decodeStreamGo : Nat → Bytes → List Bytes → Nat → Decoded
  | 0, _, _, _ => .incomplete
  | fuel + 1, buf, accRev, accC =>
    match decodeFrame buf with
    | .incomplete => .incomplete
    | .error => .error
    | .terminal c => .complete accRev.reverse.flatten (accC + c)
    | .chunk data c => decodeStreamGo fuel (buf.drop c) (data :: accRev) (accC + c)

/-- The O(1)-stack entry point for the strict chunked decode; `fuel` is the
worst-case frame count. Proven equal to `decodeStream` (`decodeStreamImpl_eq`)
and wired in as its compiled implementation. -/
def decodeStreamImpl (buf : Bytes) : Decoded :=
  decodeStreamGo (buf.length + 1) buf [] 0
...
/-- The tail-recursive engine, generalised over its accumulators, computes what
the stack-recursive `decodeStream` computes: the accumulated prefix is prepended
to the decoded body and the accumulated count added to the consumed octets.
Induction on `fuel`, kept strictly ahead of `buf.length`. -/
theorem decodeStreamGo_eq :
    ∀ (fuel : Nat) (buf : Bytes) (accRev : List Bytes) (accC : Nat),
      buf.length < fuel →
      decodeStreamGo fuel buf accRev accC =
        (match decodeStream buf with
         | .complete body c => .complete (accRev.reverse.flatten ++ body) (accC + c)
         | .incomplete => .incomplete
         | .error => .error) := by
  intro fuel
  induction fuel with
  | zero => intro buf accRev accC h; omega
  | succ fuel ih =>
    intro buf accRev accC h
    match hfr : decodeFrame buf with
    | .incomplete =>
      rw [decodeStream_incomplete buf hfr]
      simp only [decodeStreamGo, hfr]
    | .error =>
      have he : decodeStream buf = .error := by rw [decodeStream, hfr]
      rw [he]
      simp only [decodeStreamGo, hfr]
    | .terminal c =>
      rw [decodeStream_terminal buf c hfr]
      simp only [decodeStreamGo, hfr, List.append_nil]
    | .chunk data c =>
      have hb := decodeFrame_chunk_bound buf data c hfr
      have hlen : (buf.drop c).length < fuel := by
        simp only [List.length_drop]; omega
      simp only [decodeStreamGo, hfr]
      rw [ih (buf.drop c) (data :: accRev) (accC + c) hlen,
          decodeStream_chunk buf data c hfr]
      cases hrec : decodeStream (buf.drop c) with
      | complete body c' =>
        simp only [List.reverse_cons, List.flatten_concat, List.append_assoc,
                   Nat.add_assoc]
      | incomplete => rfl
      | error => rfl

/-- **`decodeStreamImpl_eq`.** The tail-recursive compiled implementation agrees
with the specification on every buffer, so `@[implemented_by decodeStreamImpl]`
is sound: no observable behaviour changes, only the native stack usage. -/
theorem decodeStreamImpl_eq (buf : Bytes) : decodeStreamImpl buf = decodeStream buf := by
  rw [decodeStreamImpl, decodeStreamGo_eq (buf.length + 1) buf [] 0 (by omega)]
  cases decodeStream buf <;> simp
...
theorem dropExt_eq_self : ∀ (bs : Bytes), (∀ b ∈ bs, b ≠ SEMI) → dropExt bs = bs := by
  ...
  | cons a t ih =>
    intro h
    have ha : a ≠ SEMI := h a (List.mem_cons_self)
...
/-- Tail-recursive engine behind `decodeStreamExt`, so the framing-grade decode
runs in O(1) native stack. `fuel` bounds the frame count (each data chunk
consumes ≥ 1 octet, so `buf.length` steps always suffice); `accRev` holds the
decoded chunk payloads seen so far in reverse and `accC` their consumed-octet
total. The self-call is in tail position, so the compiler emits a loop and a
body with unboundedly many chunks no longer grows the stack. -/
def decodeStreamExtGo : Nat → Bytes → List Bytes → Nat → Decoded
  | 0, _, _, _ => .incomplete
  | fuel + 1, buf, accRev, accC =>
    match decodeFrameExt buf with
    | .incomplete => .incomplete
    | .error => .error
    | .terminal c => .complete accRev.reverse.flatten (accC + c)
    | .chunk data c => decodeStreamExtGo fuel (buf.drop c) (data :: accRev) (accC + c)

/-- The O(1)-stack entry point for the framing-grade chunked decode; `fuel` is
the worst-case frame count. Proven equal to `decodeStreamExt`
(`decodeStreamExtImpl_eq`) and wired in as its compiled implementation. -/
def decodeStreamExtImpl (buf : Bytes) : Decoded :=
  decodeStreamExtGo (buf.length + 1) buf [] 0
...
/-- The tail-recursive engine, generalised over its accumulators, computes what
the stack-recursive `decodeStreamExt` computes: the accumulated prefix is
prepended to the decoded body and the accumulated count added to the consumed
octets. Induction on `fuel`, kept strictly ahead of `buf.length`. -/
theorem decodeStreamExtGo_eq :
    ∀ (fuel : Nat) (buf : Bytes) (accRev : List Bytes) (accC : Nat),
      buf.length < fuel →
      decodeStreamExtGo fuel buf accRev accC =
        (match decodeStreamExt buf with
         | .complete body c => .complete (accRev.reverse.flatten ++ body) (accC + c)
         | .incomplete => .incomplete
         | .error => .error) := by
  intro fuel
  ...
  | succ fuel ih =>
    intro buf accRev accC h
    match hfr : decodeFrameExt buf with
    | .incomplete =>
      rw [decodeStreamExt_incomplete buf hfr]
      simp only [decodeStreamExtGo, hfr]
    | .error =>
      have he : decodeStreamExt buf = .error := by rw [decodeStreamExt, hfr]
      rw [he]
      simp only [decodeStreamExtGo, hfr]
    | .terminal c =>
      rw [decodeStreamExt_terminal buf c hfr]
      simp only [decodeStreamExtGo, hfr, List.append_nil]
    | .chunk data c =>
      have hb := decodeFrameExt_chunk_bound buf data c hfr
      have hlen : (buf.drop c).length < fuel := by
        simp only [List.length_drop]; omega
      simp only [decodeStreamExtGo, hfr]
      rw [ih (buf.drop c) (data :: accRev) (accC + c) hlen,
          decodeStreamExt_chunk buf data c hfr]
      cases hrec : decodeStreamExt (buf.drop c) with
      | complete body c' =>
        simp only [List.reverse_cons, List.flatten_concat, List.append_assoc,
                   Nat.add_assoc]
      | incomplete => rfl
...
/-- **`decodeStreamExtImpl_eq`.** The tail-recursive compiled implementation
agrees with the specification on every buffer, so
`@[implemented_by decodeStreamExtImpl]` is sound: no observable behaviour
changes, only the native stack usage. -/
theorem decodeStreamExtImpl_eq (buf : Bytes) :
    decodeStreamExtImpl buf = decodeStreamExt buf := by
  rw [decodeStreamExtImpl, decodeStreamExtGo_eq (buf.length + 1) buf [] 0 (by omega)]
  cases decodeStreamExt buf <;> simp
