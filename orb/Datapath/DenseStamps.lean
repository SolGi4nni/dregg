...
theorem meteredAttrs_no_client (peer : Bytes) (seq : Nat) (input : Bytes) :
  ...
  Reactor.Deploy.ctxOfMetered_find_none peer seq input clientKey
    (by decide) (by decide)
...
#guard (serveMeteredPlus2HeadIdx cleanPeer 512 bulkDemoReq).data.toList
        == (drorbServeMeteredPlus2 cleanPeer 512 bulkDemoReq).data.toList
...
#guard (serveMeteredPlus2HeadIdx cleanPeer 512 bulkDemoReq).size < 1024
