...
deployed rate gate admits — the bucket is full by construction. The ctx carries no
configured burst parameters and no accept-path readings, so it reads
`defaultBurstCap` at count `0` and clock `0`: a full bucket, admitted. -/
theorem rate_admits_reqCtx (req : Proto.Request) :
    Reactor.Stage.Rate.admits (reqCtx req) = true := by
  have hc : Reactor.Stage.Rate.burstCapOf (reqCtx req)
      = Reactor.Stage.Rate.defaultBurstCap := rfl
  have hs : Reactor.Stage.Rate.seqOf (reqCtx req) = 0 := rfl
  have hk : Reactor.Stage.Rate.clockOf (reqCtx req) = 0 := rfl
  show Reactor.Stage.Rate.admitsAt (Reactor.Stage.Rate.burstCapOf (reqCtx req))
      (Reactor.Stage.Rate.burstRateOf (reqCtx req))
      (Reactor.Stage.Rate.seqOf (reqCtx req)) (Reactor.Stage.Rate.clockOf (reqCtx req)) = true
  rw [hc, hs, hk, Reactor.Stage.Rate.admitsAt_iff _ _ _ (by decide)]
  simp [Reactor.Stage.Rate.defaultBurstCap]
