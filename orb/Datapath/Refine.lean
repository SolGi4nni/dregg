...
theorem byteArray_get!_set!_ne (b : ByteArray) (j k : Nat) (v : UInt8) (hne : k ≠ j) :
  ...
  show (b.data.set! j v)[k]! = b.data[k]!
  by_cases hk : k < b.data.size
  · have hb : k < (b.data.set! j v).size := by simpa [Array.set!] using hk
    rw [getElem!_pos (b.data.set! j v) k hb, getElem!_pos b.data k hk]
    simp only [Array.set!]
    exact Array.getElem_setIfInBounds_ne hk (Ne.symm hne)
  · have hb : ¬ k < (b.data.set! j v).size := by simpa [Array.set!] using hk
    rw [getElem!_neg (b.data.set! j v) k hb, getElem!_neg b.data k hk]
...
theorem byteArray_get!_set!_self (b : ByteArray) (j : Nat) (v : UInt8) (h : j < b.size) :
  ...
  show (b.data.set! j v)[j]! = v
  have hj : j < b.data.size := h
  have hb : j < (b.data.set! j v).size := by simpa [Array.set!] using hj
  rw [getElem!_pos (b.data.set! j v) j hb]
  simp only [Array.set!, Array.getElem_setIfInBounds_self]
