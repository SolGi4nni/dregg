...
def conformantServeFast (inner : ByteArray → ByteArray) (input : ByteArray) : ByteArray :=
  ...
  | none =>
    -- The ACME HTTP-01 challenge arm. `conformantServe` grew this branch after
    -- this fast twin was written, and because neither this module nor
    -- `Datapath.ServeConformantFused` was reachable from any Lake target, the
    -- resulting divergence went unbuilt and unnoticed: under `DRORB_SPAN=22` a
    -- `/.well-known/acme-challenge/<token>` GET fell through to the ordinary
    -- pipeline instead of returning the key authorization, so issuance could not
    -- complete on this span. Mirroring the branch here is what makes
    -- `conformantServeFast_eq` true rather than weakening it.
...
/-! ## Axiom audit — expect ⊆ {propext, Quot.sound, Classical.choice}. -/
