...
def lenExtra (sym : Nat) (bits : Bits) : Option (Nat × Bits) :=
  match lengthTable[(sym - 257)]? with
  | none => none
  | some (base, ex) =>
...
def copyMatch (maxOut dist : Nat) : Nat → Array UInt8 → Option (Array UInt8)
  ...
  | n + 1, out =>
    ...
    else copyMatch maxOut dist n (out.push (out[(out.size - dist)]!))
  ...
  | some (hlit, b1) =>
    ...
    | some (hdist, b2) =>
      ...
      | some (hclen, b3) =>
        ...
        let numDist := hdist + 1
        let numCL := hclen + 4
        match readCLLens numCL 0 (Array.replicate 19 0) b3 with
        | none => none
        | some (clArr, b4) =>
...
theorem copyMatch_le (maxOut dist : Nat) :
  ...
  | succ k ih =>
    ...
    · rw [if_neg hge] at h
      ...
      · rw [if_neg hd] at h
        have hpush : (out.push (out[(out.size - dist)]!)).size ≤ maxOut := by
