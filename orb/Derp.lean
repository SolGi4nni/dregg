...
private theorem u8_toNat_ofNat (n : Nat) : (UInt8.ofNat n).toNat = n % 256 := rfl
...
theorem derp_frame_bounds (maxLen : Nat) (bs : Bytes) {f : Frame}
  ...
  -- Only the ≥5-byte shape can produce `some`; fewer bytes give `none`.
  match bs with
  | [] | [_] | [_, _] | [_, _, _] | [_, _, _, _] =>
  ...
  | t :: l0 :: l1 :: l2 :: l3 :: tl =>
    ...
    · rename_i hcond
      ...
      obtain ⟨hf, hrest⟩ := h
      -- Project the payload out of `hf` rather than `subst`ing `f`: substituting
      -- the whole `Frame` literal drags `FrameType.ofByte t` (a 17-way `UInt8`
      -- match) into every goal, and normalizing that under the imported
      -- environment overflows the kernel's recursion guard.
      have hpay : f.payload = tl.take (be32 l0 l1 l2 l3) := by rw [← hf]
      subst hrest
      rw [hpay]
      ...
      have h5 : (t :: l0 :: l1 :: l2 :: l3 :: tl).take headerLen = [t, l0, l1, l2, l3] := rfl
      refine ⟨?_, ?_, ?_⟩
      · -- payload.length ≤ maxLen
        rw [hplen]; exact hcap
      · -- header ++ payload ++ rest reconstruction: rewrite `take_append_drop` on
        -- a concrete 5-byte prefix so `rfl` never has to evaluate `tl.take (be32 …)`.
        rw [h5, List.append_assoc, List.take_append_drop]
        rfl
      · -- length accounting
...
theorem derp_serverinfo_authentic (serverPub clientSec : ByteArray) (payload : Bytes)
  ...
  simp only [openServerInfo, Option.map_eq_some_iff] at h
