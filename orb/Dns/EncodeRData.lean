...
/-- 4.30 drift shim:  is now keyed on  literals,
so it no longer rewrites the applied . This is the definitional fact. -/
...
theorem beNat_putBe (w : Nat) : ∀ n, n < 256 ^ w → beNat (putBe w n) = n := by
  ...
  | succ w ih =>
    ...
    have hdiv : n / 256 < 256 ^ w := by
      ...
      omega
    simp only [putBe]
    rw [beNat_append_byte, ih _ hdiv, u8_toNat_ofNat]
    omega
  ...
  | cons s tail ih =>
    ...
    | Nat.succ fuel =>
      ...
      have htn : (UInt8.ofNat s.length).toNat = s.length := by
        rw [u8_toNat_ofNat]; omega
      rw [htn]
      have hle : s.length ≤ (s ++ encodeCharStrings tail).length := by
        simp [List.length_append]
      rw [if_pos hle, List.drop_left, List.take_left]
...
theorem typedRData_encode_dnskey (msg : Bytes) (nm : List (List UInt8)) (cls ttl off : Nat)
  ...
  have hpp : (UInt8.ofNat p).toNat = p := by rw [u8_toNat_ofNat]; omega
  have haa : (UInt8.ofNat a).toNat = a := by rw [u8_toNat_ofNat]; omega
...
theorem typedRData_encode_ds (msg : Bytes) (nm : List (List UInt8)) (cls ttl off : Nat)
  ...
  have haa : (UInt8.ofNat a).toNat = a := by rw [u8_toNat_ofNat]; omega
  have hdd : (UInt8.ofNat dt).toNat = dt := by rw [u8_toNat_ofNat]; omega
  simp only [h16, List.getD_cons_succ, List.getD_cons_zero, List.drop_succ_cons,
...
theorem typedRData_encode_nsec3param (msg : Bytes) (nm : List (List UInt8)) (cls ttl off : Nat)
  ...
  have halg' : (UInt8.ofNat alg).toNat = alg := by rw [u8_toNat_ofNat]; omega
  have hfl' : (UInt8.ofNat fl).toNat = fl := by rw [u8_toNat_ofNat]; omega
