...
/-! **Determinism is not a theorem for a `def`.** `step : Job → Event → _` is a
function, so "equal inputs give equal outputs" is congruence, not a property of
`step`. The theorem that stood here, `step_deterministic : step j e = step j e`,
was exactly that non-statement. The real facts about the step are
`failSoft_attempts_le` above and `resume_reassembles` below. -/
