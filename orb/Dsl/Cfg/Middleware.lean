...
theorem chainBuilder_runState (acc : List Stage) (ss : List SStage) :
  ...
  | nil =>
    ...
    rw [List.append_nil]
  | cons a rest ih =>
    show chainBuilder.runState (acc ++ [a.val]) rest
        = acc ++ List.map Subtype.val (a :: rest)
    rw [ih]
    change (acc ++ [a.val]) ++ List.map Subtype.val rest
        = acc ++ (a.val :: List.map Subtype.val rest)
...
theorem exists_wrap {chain : List Stage} (h : chainInv chain) :
  ...
  | nil => exact ⟨[], rfl⟩
  | cons a rest ih =>
    have ha : Stage.statusStable a := h a (List.mem_cons_self)
    have hrest : chainInv rest := fun s hs => h s (List.mem_cons_of_mem a hs)
    obtain ⟨ss, hss⟩ := ih hrest
    exact ⟨(⟨a, ha⟩ : SStage) :: ss, by
      change a :: List.map Subtype.val ss = a :: rest
      rw [hss]⟩
