...
def utf8Ok (bs : Bytes) : Bool := ByteArray.validateUTF8 (ByteArray.mk bs.toArray)
...
theorem decodeLines_wf (hd : HuffmanDecoder) (st : Store) (bs : Bytes)
  ...
  induction st, bs, pseudo, acc using decodeLines.induct hd dyn base with
  | case1 st pseudo acc =>
    intro r hwf h
...
theorem decPrefixInt_encPrefixInt (p pat v : Nat) (tail : Bytes)
  ...
  have hpow : (0 : Nat) < 2 ^ p := Nat.pow_pos (by omega)
  unfold encPrefixInt
