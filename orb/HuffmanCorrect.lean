...
theorem step_val (v n : Nat) (hn : 1 ≤ n) :
  ...
  have h2 : (if nbit v (n - 1) then (1 : Nat) else 0) = (v >>> (n - 1)) % 2 := by
    simp only [nbit]
    rcases Nat.mod_two_eq_zero_or_one (v >>> (n - 1)) with h | h <;> simp [h]
  rw [h1, h2]; omega
...
theorem overlong_pad_rejected : huffmanDecode [0xff] = none := by cbv
