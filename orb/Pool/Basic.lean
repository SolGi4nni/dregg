  ...
  | none =>
    ...
    have : ∀ fuel start, start ≤ i → i < start + fuel →
      ...
      | succ fuel ih =>
        ...
        · rw [if_pos hc]; exact fun h => absurd h (by simp)
        · rw [if_neg hc]
...
protected theorem WF.empty : (empty : BufferPool).WF where
  exact_cap := by intro i c h; exact absurd h (List.not_mem_nil)
  ...
  | some i =>
    ...
    | cons c rest =>
      ...
      have hc : c = classSize i :=
        h.exact_cap i c (by rw [hb]; exact List.mem_cons_self)
      rw [hc]
...
theorem get_class_exact {p : BufferPool} (h : p.WF) {n i : Nat}
  ...
  | cons c rest =>
    rw [get_hit hi hb]
    exact h.exact_cap i c (by rw [hb]; exact List.mem_cons_self)
