...
protected theorem WF.init (smallCap largeCap maxPooled : Nat) :
  ...
  small_fit := by intro c h; exact absurd h (List.not_mem_nil)
  large_fit := by intro c h; exact absurd h (List.not_mem_nil)
...
theorem takeSmall_cap {p : RecvPool} (h : p.WF) :
  ...
  | cons c rest =>
    rw [takeSmall_pop hs]
    exact (h.small_fit c (by rw [hs]; exact List.mem_cons_self)).1
...
theorem takeLarge_cap {p : RecvPool} (h : p.WF) :
  ...
  | cons c rest =>
    rw [takeLarge_pop hs]
    exact h.large_fit c (by rw [hs]; exact List.mem_cons_self)
