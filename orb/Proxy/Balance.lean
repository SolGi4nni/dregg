...
theorem wleastConn_uniform {bs : List Backend} {k : Nat} (hk : 0 < k)
  ...
  | cons a rest ih =>
    ...
    | some c =>
      have hac : a.weight = k := hw a (List.mem_cons_self)
...
theorem minTier_mem {bs : List Backend} {t : Nat} (h : minTier bs = some t) :
  ...
  | cons a rest ih =>
    ...
    | some t' =>
      ...
      · exact ⟨a, List.mem_cons_self, (Nat.min_eq_left hmin).symm⟩
...
theorem select_wrr_total {ctx : Ctx} {bs : List Backend} {w : Backend}
  ...
  | cons c rest =>
    have hcmem : c ∈ tierPool bs := by
      rw [hpool]; exact List.mem_cons_self
