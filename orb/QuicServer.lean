...
def buildShortPacket (dcid : ByteArray) (pn : Nat) (spin keyPhase : Bool)
  ...
  let payload := if payload0.size < 4 then
      payload0 ++ ByteArray.mk (Array.replicate (4 - payload0.size) 0) else payload0
...
theorem quicKex_group_always_hybrid {ch : ClientHello}
  ...
  | none => rw [quicKex_requires_hybrid hh] at hok; exact absurd hok (by simp)
...
def buildFlightFromCH (wireDcid origDcid cscid chMsg : ByteArray) (recvPns : List Nat)
  ...
  let initPayload := initPayload0 ++ ByteArray.mk (Array.replicate deficit 0)
...
theorem find?_eq_of_mem_unique {α} (p : α → Bool) (l : List α) (a : α)
  ...
  | cons x xs ih =>
    ...
    · have hxa : x = a := huniq x (.head _) hx
      subst hxa
      simp [List.find?_cons_of_pos hx]
    · have hax : a ≠ x := fun e => hx (e ▸ hp)
      have ha' : a ∈ xs := by
        rcases List.mem_cons.mp ha with h | h
        · exact absurd h hax
        · exact h
      rw [List.find?_cons_of_neg (by simpa using hx)]
