...
```text
  server SETTINGS                  -- RFC 9113 §3.4 server connection preface
                                   --   (advertises MAX_CONCURRENT_STREAMS, §6.5.2)
...
/-- An **empty SETTINGS frame** (RFC 9113 §6.5).
`00 00 00 | 04 | 00 | 00 00 00 00` — length 0, type SETTINGS (0x4), flags 0,
stream 0. The runtime guards below use it as the CLIENT's SETTINGS in a
realistic opening burst. (The server's own connection preface is
`H2.Conn.serverSettings`, which additionally advertises
`SETTINGS_MAX_CONCURRENT_STREAMS`.) -/
def emptySettings : Bytes :=
...
/-! The client-side empty SETTINGS decodes, through the REAL `H2.decode`, as a
SETTINGS frame on stream 0 (not an ACK). -/
#guard H2.decode emptySettings Reactor.H2.h2MaxFrameSize
  = .complete (.settings 0 false []) 9

/-! The server connection preface (RFC 9113 §3.4) decodes as a SETTINGS frame
on stream 0 (not an ACK) whose one 6-octet entry is
`SETTINGS_MAX_CONCURRENT_STREAMS = 100` (identifier 0x3, §6.5.1/§6.5.2). -/
#guard H2.decode H2.Conn.serverSettings Reactor.H2.h2MaxFrameSize
  = .complete (.settings 0 false [0x00, 0x03, 0x00, 0x00, 0x00, 0x64]) 15
...
SETTINGS preface (15 octets — it advertises `SETTINGS_MAX_CONCURRENT_STREAMS`),
the SETTINGS ACK (9 octets), then a response that
...
#guard
  (Reactor.H2Response.decodeResponse
    ((serveH2c (H2.Conn.clientPreface
        ++ emptySettings ++ healthHeadersFrame)).drop 24))
