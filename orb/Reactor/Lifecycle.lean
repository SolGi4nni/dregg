...
/-! **Totality is not a theorem in Lean.** `lifecycleServe : DState → List Header.Op → Bytes → _` is a plain `def`, so
...
`lifecycleServe_total : lifecycleServe d prog input = lifecycleServe d prog input`, was exactly that non-statement:
it holds of EVERY function of the same arity, so it constrained `lifecycleServe` not at
all while wearing a name that claimed it did. The real facts about the drain-aware serve are the theorems above. -/
