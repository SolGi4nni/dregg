...
/-- The tier pool of the loaded fleet carries the same backends, in the same
order, as the tier pool of the zero-load fleet: `conns` touches neither
`eligible` (health ∧ status) nor `tier`, which are all `tierPool` looks at. -/
theorem tierPool_fleetC_ids (mask : Nat) (conns : Nat → Nat) :
    (Proxy.tierPool (fleetC mask conns)).map Backend.id
      = (Proxy.tierPool (fleet mask)).map Backend.id := by
  cases h0 : mask.testBit 0 <;> cases h1 : mask.testBit 1 <;> cases h2 : mask.testBit 2 <;>
    simp [Proxy.tierPool, Proxy.bestTier, Proxy.eligibleOf, Proxy.minTier,
      fleet, fleetC, Backend.eligible, h0, h1, h2]

/-- **Stickiness.** The pick is a function of the live health mask and the
affinity key ALONE: an arbitrary change to the per-backend in-flight load moves
no key. So a fixed session key stays on its backend across requests while traffic
shifts under it — only a MEMBERSHIP change can move it, and that is
`pick_health_ejects`.

This is the real content of "sticky": a load-sensitive selector (least-conns)
does NOT satisfy it, and `pick mask key = pick mask key` — which held of every
function — did not say it. -/
theorem pick_sticky (mask key : Nat) (conns : Nat → Nat) :
    pickLoaded dialPolicies mask conns key = pick mask key := by
  unfold pickLoaded pick pickBackend dialPolicies Proxy.selectChain
  simp only [Proxy.select, Proxy.applyPolicy]
  cases hL : Proxy.rendezvous (mkCtx key).hash (mkCtx key).key
      (Proxy.tierPool (fleetC mask conns)) with
  | none =>
    cases hR : Proxy.rendezvous (mkCtx key).hash (mkCtx key).key
        (Proxy.tierPool (fleet mask)) with
    | none => rfl
    | some b =>
      have := Proxy.rendezvous_ignores_load (hash := (mkCtx key).hash)
        (key := (mkCtx key).key) (tierPool_fleetC_ids mask conns)
      rw [hL, hR] at this; simp at this
  | some a =>
    cases hR : Proxy.rendezvous (mkCtx key).hash (mkCtx key).key
        (Proxy.tierPool (fleet mask)) with
    | none =>
    ...
    | some b =>
      ...
      exact congrArg some (by simpa using this)
...
`pick_sticky` used to read `pick mask key = pick mask key`, which holds of every
function; it is now load-independence of the rendezvous pick. `#assert_nonvacuous`
pins that: if the statement ever decays back into `a = a`, this file goes red. -/

#assert_nonvacuous pick_sticky
#assert_axioms pick_sticky ⊆ [stdAxioms]
#assert_axioms pickWith_health_ejects ⊆ [stdAxioms]
