...
/-! **Totality is not a theorem in Lean.** `step : QuicConfig → QuicState → DatagramEvent → _` is a plain `def`, so
...
`step_total : step cfg st e = step cfg st e`, was exactly that non-statement:
it holds of EVERY function of the same arity, so it constrained `step` not at
all while wearing a name that claimed it did. The real facts about the step are the QUIC/H3 seam theorems below. -/
