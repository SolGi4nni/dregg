...
/-! **Totality is not a theorem in Lean.** `datagramServe : QuicConfig → QuicState → DatagramEvent → _` is a plain `def`, so
...
`datagramServe_total : datagramServe cfg st e = datagramServe cfg st e`, was exactly that non-statement:
it holds of EVERY function of the same arity, so it constrained `datagramServe` not at
all while wearing a name that claimed it did. The real fact about the ingress is `quic_ingress_dispatch` below. -/
