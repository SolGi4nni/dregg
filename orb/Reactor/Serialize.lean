...
* (there is no `serialize_total`: totality of a `def` is a type-level fact, not a
  stuck state.
-/
...
/-! **Totality is not a theorem in Lean.** `serialize : Response → Bytes` is a plain `def`, so
...
`serialize_total : serialize resp = serialize resp`, was exactly that non-statement:
it holds of EVERY function of the same arity, so it constrained `serialize` not at
all while wearing a name that claimed it did. The real facts about the render are `serialize_body_suffix` above and the `serializeFast` byte-identity theorems below. -/
