...
import Arena.Parse
import Reactor.Stage.AcmeChallenge
...
theorem get!_eq_getElem (bs : ByteArray) (i : Nat) (h : i < bs.size) :
  ...
  rw [show bs.get! i = bs.data[i]! from rfl, getElem!_pos bs.data i h]
...
/-- **The HTTP-01 challenge route, landed on the deployed serve** (RFC 8555 §8.3).
If the parsed request is a `GET /.well-known/acme-challenge/<token>`, this answers
the serialized `200` (+`Date`) whose body is the RFC §8.1 key authorization for
`<token>` under the deployed account key — the exact value a validating CA
recomputes and compares (`Reactor.Stage.AcmeChallenge.challenge_body_ca_accepted`,
over the REAL `Crypto.sha256`). Every other request yields `none`, and
`conformantServe` routes it to `inner` byte-for-byte unchanged. The route is
inner-independent, so the whole dense/head-idx `= drorbServe` family carries
through `conformantServe_congr`. -/
def acmeChallengeBytes (input : ByteArray) : Option Bytes :=
  match Proto.RequestSerialize.parse (reqBytes input) with
  | some req =>
      (Reactor.Stage.AcmeChallenge.challengeResponse
        Reactor.Stage.AcmeChallenge.deployAcctKey req).map
        (fun r => serialize (addDate r))
  | none => none

/-- **The conformant serve (DENSE + Z1 + N2 + HTTP-01).** The pre-parse HEAD gate
(Z1 — `414 URI Too Long` for an over-long request-line, `431` for an oversized
header block, decided by bounded index probes BEFORE the recursive parse can
overflow; a large BODY behind a small head passes) runs OUTERMOST, so it keeps
priority over every route. Past it, the HTTP-01 challenge route is consulted: a
(head-bounded) `GET /.well-known/acme-challenge/<token>` is answered with the key
authorization (RFC 8555 §8.3) and never reaches `inner`. Otherwise it wraps `inner`
with the proven conformance stages: validation gate (C1/C2/B2/G1/C3) → `inner` →
`Date` (F1, dense splice) → `x-corr` head scrub (N2, dense — no amplification /
request-byte disclosure) / `HEAD`-strip (B1, dense truncate). The accepted-path
body is carried as `ByteArray` throughout — no 1 MiB `List` re-cons
(`conformantServe_toList` pins the bytes to the list spec). -/
def conformantServe (inner : ByteArray → ByteArray) (input : ByteArray) : ByteArray :=
  ...
  | none =>
    match acmeChallengeBytes input with
    | some out => ByteArray.mk out.toArray
    | none =>
...
theorem conformantServe_toList (inner : ByteArray → ByteArray) (input : ByteArray) :
  ...
  · rcases ha : acmeChallengeBytes input with _ | out
    · simp only []
...
theorem conformantServe_head_bytes
    ...
    (hacme : acmeChallengeBytes input = none) :
    (conformantServe inner input).toList
      = stripBody (scrubCorr (respBytesRaw inner input)) := by
  rw [conformantServe_toList]
  simp only [hacme, hz, if_pos hhead]
...
theorem teInput_parses : Proto.RequestSerialize.parse (reqBytes teInput) = some teReq := by
  have h1 : teInput.toList = Proto.RequestSerialize.serialize teReq := by
    rw [ba_toList_eq teInput]; exact List.toList_toArray
...
theorem exInput_parses : Proto.RequestSerialize.parse (reqBytes exInput) = some exReq := by
  have h1 : exInput.toList = Proto.RequestSerialize.serialize exReq := by
    rw [ba_toList_eq exInput]; exact List.toList_toArray
...
theorem condInput_parses :
  ...
  have h1 : condInput.toList = Proto.RequestSerialize.serialize reqINM := by
    rw [ba_toList_eq condInput]; exact List.toList_toArray
...
theorem condInputIMS_parses :
  ...
  have h1 : condInputIMS.toList = Proto.RequestSerialize.serialize reqIMS := by
    rw [ba_toList_eq condInputIMS]; exact List.toList_toArray
