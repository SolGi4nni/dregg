...
/-! ## ★ The bare-LF UPSTREAM-RESPONSE gate, at the EFFECT SEAM (RFC 9112 §2.2)

`parseUpstream` above splits the upstream head on CRLF **only** (`splitCRLFLines`).
...
any preceding CR."* So an upstream response head carrying a **bare LF** has TWO
admissible parses — to this core everything after the bare LF is part of the PREVIOUS
field's value; to an LF-tolerant downstream client it is a field-line of its own, and
after a bare LF LF an entirely new message. Two hops disagreeing about where a
response's fields (and hence its body) begin is HTTP response splitting (RFC 9112
§11.2), and RFC 9110 §7.6.1 makes this hop responsible for the head it hands its
client.

`Reactor.ProxyForward.forwardRespHeadGated` closed that hole on the DEFAULT proxy
path. This is the SAME decision at the OTHER response-head consumer: the effect seam
(`DRORB_EFFECT_SEAM=1`), where the reply is threaded through `proxyRespTransform`
instead of `forwardRespHead`. Measured on the SHIPPED io_uring reactor
(`conformance/proxy/respsplit_probe.py`, 2026-07-25) BEFORE this gate,
`PROXY_SEAM=1 PROXY_IO=uring` forwarded the crafted upstream head and `curl -D -` — a
real client — reported `Set-Cookie: sess=EVIL; Path=/` as a field of the response;
the second vector put a whole `HTTP/1.1 200 OK … OWNED!` message behind a bare LF LF
and the client saw two responses to one request.

The gate is the default path's, MIRRORED — same predicate (`Body.FrameRaw.noBareLF`),
same fail-closed shape (a `reject` that carries NO bytes), same reject surface
(`gatewayError false` = `502 Bad Gateway`, RFC 9110 §15.6.3: an upstream response this
intermediary cannot forward unambiguously IS an invalid response from the inbound
server). It differs in exactly one place, and that difference is forced: the seam hands
`proxyRespTransform` the WHOLE upstream reply, so the predicate is applied to the
reply's HEAD BLOCK (`upstreamHeadBlock` — the bytes before the first CRLFCRLF) and NOT
to the body. A response BODY may legitimately carry bare LFs (the probe's own `ok\n`
does); gating those would refuse every honest reply, and a bare LF in the body cannot
forge a field-line because the head is already terminated before it. -/
...
It lives HERE rather than in `Reactor.ProxyForward` (where it was written for the
default proxy path) so the default path's refusal and the seam path's refusal are ONE
definition that cannot drift apart; `Reactor.ProxyForward.gatewayError` IS this. -/
...
/-- The HEAD BLOCK of an upstream reply: the bytes before the first CRLFCRLF — exactly
the bytes `parseUpstream` runs `splitCRLFLines` over. This, not the whole reply, is what
the bare-LF gate decides on. -/
def upstreamHeadBlock (upstream : Bytes) : Bytes := (splitHeadBody upstream).1

/-- The verdict on an upstream reply at the effect seam: either the transformed bytes to
send, or a refusal. There is no third outcome and no "forward anyway" — the refusal
carries no bytes, which is what makes the failure closed. The shape mirrors
`Reactor.ProxyForward.RespHeadOutcome` (that inductive lives in a module that imports
this one, so it cannot be reused here). -/
inductive UpstreamOutcome where
  /-- The upstream head is not unambiguously terminated (RFC 9112 §2.2): refuse it. The
  client is answered `502 Bad Gateway` (`gatewayError false`). -/
  | reject : UpstreamOutcome
  /-- The transformed response bytes to send. -/
  | forward : Bytes → UpstreamOutcome
  deriving DecidableEq, Repr

/-- **The gated seam response transform.** Refuse an upstream reply whose HEAD BLOCK has
ambiguous line terminators (`noBareLF`, RFC 9112 §2.2); otherwise take the proven
transform `proxyRespTransform` unchanged. This is the decision the seam's proxy
continuation crosses instead of `proxyRespTransform` directly. -/
def proxyRespGate (input upstream : Bytes) : UpstreamOutcome :=
  if noBareLF (upstreamHeadBlock upstream) then
    .forward (proxyRespTransform input upstream)
  else .reject

/-- The wire bytes of a verdict: a refusal is the `502`, a forward is its own bytes. -/
def renderUpstreamOutcome : UpstreamOutcome → Bytes
  | .reject      => gatewayError false
  | .forward out => out

/-- **The bytes the seam's proxy continuation `.done`s.** `proxyRespTransform` behind the
RFC 9112 §2.2 bare-LF refusal. `proxyRespTransform` itself is KEPT (ungated) as the object
every existing transform theorem is stated over, exactly as `forwardRespHead` was kept on
the default path; `seamGate_ungated_differs` machine-checks that re-pointing the seam back
at it restores the leak. -/
def proxyRespTransformGated (input upstream : Bytes) : Bytes :=
  renderUpstreamOutcome (proxyRespGate input upstream)

/-- **The gate fails closed on a bare LF.** An upstream reply whose head block carries a
bare LF is refused — the gate never yields bytes to forward, so none of that head reaches
the client. -/
theorem seamGate_bareLF_rejected (input upstream : Bytes)
    (hbare : noBareLF (upstreamHeadBlock upstream) = false) :
    proxyRespGate input upstream = .reject
    ∧ (∀ out, proxyRespGate input upstream ≠ .forward out) := by
  have hr : proxyRespGate input upstream = .reject := by
    simp [proxyRespGate, hbare]
  exact ⟨hr, by intro out h; rw [hr] at h; exact UpstreamOutcome.noConfusion h⟩

/-- **★ The gate forwards only unambiguously-terminated upstream heads.** Whenever the gate
hands the seam bytes to send, the upstream head block contained no bare LF — so this core's
CRLF-only parse is the only parse RFC 9112 §2.2 permits of it. The completeness direction,
mirroring `Reactor.ProxyForward.respGate_forward_noBareLF`. -/
theorem seamGate_forward_noBareLF (input upstream out : Bytes)
    (h : proxyRespGate input upstream = .forward out) :
    noBareLF (upstreamHeadBlock upstream) = true := by
  cases hclean : noBareLF (upstreamHeadBlock upstream) with
  | false => exact absurd h ((seamGate_bareLF_rejected input upstream hclean).2 out)
  | true  => rfl

/-- On a reply whose head block has unambiguous terminators the gate is **exactly** the
proven transform — the refusal adds a refusal and changes nothing else. -/
theorem seamGate_clean (input upstream : Bytes)
    (hclean : noBareLF (upstreamHeadBlock upstream) = true) :
    proxyRespGate input upstream = .forward (proxyRespTransform input upstream) := by
  simp [proxyRespGate, hclean]

/-- **A reject is not a forward.** Stated on its own because "fail closed" is the whole
point: there is no byte string — not even the empty one — that a refused upstream reply
yields as a forward. -/
theorem seamGate_reject_is_not_forward (input upstream : Bytes)
    (h : proxyRespGate input upstream = .reject) (out : Bytes) :
    proxyRespGate input upstream ≠ .forward out := by
  rw [h]; intro hc; exact UpstreamOutcome.noConfusion hc

/-- The refused reply's WIRE bytes: the `502`, never any part of the upstream head. -/
theorem seamGate_bytes_rejected (input upstream : Bytes)
    (hbare : noBareLF (upstreamHeadBlock upstream) = false) :
    proxyRespTransformGated input upstream = gatewayError false := by
  simp [proxyRespTransformGated, (seamGate_bareLF_rejected input upstream hbare).1,
    renderUpstreamOutcome]

/-- **No regression.** On a clean upstream head the gated seam transform is the ungated
one, byte-for-byte: the gate changes nothing for honest traffic. -/
theorem seamGate_bytes_clean (input upstream : Bytes)
    (hclean : noBareLF (upstreamHeadBlock upstream) = true) :
    proxyRespTransformGated input upstream = proxyRespTransform input upstream := by
  simp [proxyRespTransformGated, seamGate_clean input upstream hclean, renderUpstreamOutcome]

/-- The refusal surfaces as a `502` status line — not a `504`, not a pass-through. -/
theorem seamGateReject_is_502 :
    (gatewayError false).take 12 = [72, 84, 84, 80, 47, 49, 46, 49, 32, 53, 48, 50] := by decide

/-- The refusal is not the timeout response: a bare-LF upstream head is reported as a bad
gateway, never as a gateway timeout. -/
theorem seamGateReject_not_timeout : gatewayError false ≠ gatewayError true := by decide

#assert_axioms seamGate_bareLF_rejected ⊆ [stdAxioms]
#assert_axioms seamGate_forward_noBareLF ⊆ [stdAxioms]
#assert_axioms seamGate_clean ⊆ [stdAxioms]
#assert_axioms seamGate_reject_is_not_forward ⊆ [stdAxioms]
#assert_axioms seamGate_bytes_rejected ⊆ [stdAxioms]
#assert_axioms seamGate_bytes_clean ⊆ [stdAxioms]
...
def serveStep (mask : Nat) (input : Bytes) : Step :=
  ...
  | true =>
    ...
    | some id => .yield (.proxyDial id input) (fun up => .done (proxyRespTransformGated input up))
...
/-- **The proxy path yields the proven-chosen backend**, with the GATED full
response-transform as its continuation (RFC 9112 §2.2 bare-LF refusal in front of the
same fold — `proxyRespTransformGated`). -/
theorem serveStep_proxy_yields (mask : Nat) (input : Bytes) (id : BackendId)
    ...
    serveStep mask input
      = .yield (.proxyDial id input) (fun up => .done (proxyRespTransformGated input up)) := by
...
/-- **The proxy continuation runs the GATED FULL response-transform fold over the
upstream reply.** After the shell dials the backend and returns `upstream`, resuming
produces `proxyRespTransformGated input upstream` — the reply refused if its head block
carries a bare LF (RFC 9112 §2.2), otherwise parsed, run through cors / gzip /
security-headers / header, and re-serialized.

RESTATED under the gate: this is the pre-gate `serveStep_proxy_resume` with the object the
seam now crosses in place of the one it used to. The pre-gate statement is not lost — it is
`serveStep_proxy_resume_clean` below, under the hypothesis that makes it true. -/
theorem serveStep_proxy_resume (mask : Nat) (input upstream : Bytes) (id : BackendId)
    (h : isApiPath input = true)
    (hpick : Reactor.ProxyDial.pick mask (stickyKey input) = some id) :
    resumeStep mask input [upstream] = .done (proxyRespTransformGated input upstream) := by
  unfold resumeStep
  rw [serveStep_proxy_yields mask input id h hpick]
  simp only [stepFeed_yield, stepFeed_nil]

/-- **★ The seam refuses a bare-LF upstream reply on the DEPLOYED path.** For a proxy
request whose proven pick finds an eligible backend, an upstream reply whose head block
carries a bare LF resumes to the `502 Bad Gateway` bytes and NOTHING else — no part of the
crafted head is in the response. This is the deployed-level rejection guarantee, the seam
analogue of `Reactor.ProxyForward.respGate_bareLF_rejected`. -/
theorem serveStep_proxy_resume_rejected (mask : Nat) (input upstream : Bytes) (id : BackendId)
    (h : isApiPath input = true)
    (hpick : Reactor.ProxyDial.pick mask (stickyKey input) = some id)
    (hbare : noBareLF (upstreamHeadBlock upstream) = false) :
    resumeStep mask input [upstream] = .done (gatewayError false) := by
  rw [serveStep_proxy_resume mask input upstream id h hpick,
      seamGate_bytes_rejected input upstream hbare]

/-- **★ No regression on honest upstreams, on the DEPLOYED path.** When the upstream head
block has unambiguous terminators, the resume produces exactly the pre-gate bytes
`proxyRespTransform input upstream`. This is the ORIGINAL `serveStep_proxy_resume`
statement, preserved verbatim under the hypothesis the gate admits on. -/
theorem serveStep_proxy_resume_clean (mask : Nat) (input upstream : Bytes) (id : BackendId)
    (h : isApiPath input = true)
    (hpick : Reactor.ProxyDial.pick mask (stickyKey input) = some id)
    (hclean : noBareLF (upstreamHeadBlock upstream) = true) :
    resumeStep mask input [upstream] = .done (proxyRespTransform input upstream) := by
  rw [serveStep_proxy_resume mask input upstream id h hpick,
      seamGate_bytes_clean input upstream hclean]
...
(see `Hygiene`), which throws on any dependency outside the declared set. All 24
serve/cache/proxy-resume theorems below are kernel-checked under the standard
three and nothing else — no FFI seam, no `native_decide`. -/

#assert_axioms serveStep_noncacheable ⊆ [stdAxioms]
#assert_axioms serveStep_cacheable ⊆ [stdAxioms]
#assert_axioms cacheResume_hit ⊆ [stdAxioms]
#assert_axioms cacheResume_hit_plain ⊆ [stdAxioms]
#assert_axioms cacheResume_hit_notModified ⊆ [stdAxioms]
#assert_axioms cacheResume_hit_preconditionFailed ⊆ [stdAxioms]
#assert_axioms cacheResume_miss ⊆ [stdAxioms]
#assert_axioms cacheResume_miss_plain ⊆ [stdAxioms]
#assert_axioms revalidate_stored ⊆ [stdAxioms]
#assert_axioms conditionalRewrite_noCond ⊆ [stdAxioms]
#assert_axioms serveStep_gate_rejects ⊆ [stdAxioms]
#assert_axioms resumeStep_cache_hit ⊆ [stdAxioms]
#assert_axioms resumeStep_cache_hit_plain ⊆ [stdAxioms]
#assert_axioms resumeStep_cache_miss ⊆ [stdAxioms]
#assert_axioms resumeStep_cache_miss_plain ⊆ [stdAxioms]
#assert_axioms serveStep_proxy_resume ⊆ [stdAxioms]
#assert_axioms proxyRespTransform_hsts ⊆ [stdAxioms]
...
theorem resumeStepWith_proxy (policies : List Proxy.Policy) (mask : Nat)
    ...
    (hpick : Reactor.ProxyDial.pickWith policies mask (stickyKey input) = some id) :
    resumeStepWith policies mask input [upstream]
      = .done (proxyRespTransformGated input upstream) := by
  unfold resumeStepWith
  rw [serveStepWith_proxy_yields policies mask input id h hpick]
  simp only [stepFeed_yield, stepFeed_nil]

/-- **★ The config-driven resume refuses a bare-LF upstream reply too** — the gate is in
the shared continuation, so EVERY LB policy chain (and therefore every deployment the DSL
can emit) inherits it, not just the deployed default. -/
theorem resumeStepWith_proxy_rejected (policies : List Proxy.Policy) (mask : Nat)
    (input upstream : Bytes) (id : BackendId) (h : isApiPath input = true)
    (hpick : Reactor.ProxyDial.pickWith policies mask (stickyKey input) = some id)
    (hbare : noBareLF (upstreamHeadBlock upstream) = false) :
    resumeStepWith policies mask input [upstream] = .done (gatewayError false) := by
  rw [resumeStepWith_proxy policies mask input upstream id h hpick,
      seamGate_bytes_rejected input upstream hbare]

/-- **★ No regression for any config chain**: a clean upstream head resumes to the
pre-gate bytes. -/
theorem resumeStepWith_proxy_clean (policies : List Proxy.Policy) (mask : Nat)
    (input upstream : Bytes) (id : BackendId) (h : isApiPath input = true)
    (hpick : Reactor.ProxyDial.pickWith policies mask (stickyKey input) = some id)
    (hclean : noBareLF (upstreamHeadBlock upstream) = true) :
...
#assert_axioms serveStepWith_default ⊆ [stdAxioms]
#assert_axioms serveStepWith_deploy ⊆ [stdAxioms]
#assert_axioms serveStepWith_proxy_yields ⊆ [stdAxioms]
#assert_axioms resumeStepWith_proxy ⊆ [stdAxioms]
#assert_axioms serveStep_proxy_resume_rejected ⊆ [stdAxioms]
#assert_axioms serveStep_proxy_resume_clean ⊆ [stdAxioms]
#assert_axioms resumeStepWith_proxy_rejected ⊆ [stdAxioms]
#assert_axioms resumeStepWith_proxy_clean ⊆ [stdAxioms]
...
theorem proxy_stream_resume_faithful (mask : Nat) (input upstream : Bytes) (id : BackendId)
  ...
  rw [serveStep_proxy_resume_clean mask input upstream id h hpick hclean]
...
#assert_axioms proxyRespTransform_split ⊆ [stdAxioms]
#assert_axioms proxyStreamResume_faithful ⊆ [stdAxioms]
#assert_axioms proxy_stream_resume_faithful ⊆ [stdAxioms]
