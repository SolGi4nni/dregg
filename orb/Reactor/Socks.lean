...
/-! ## The deployed byte seam — the real `Socks.hstep` decision, over `ByteArray`

The running SOCKS5 egress driver (`crates/dataplane/src/socks.rs`) does not
re-implement the handshake state machine: it CALLS this exported decision once
per phase and acts on the verdict. The host owns only socket I/O and the outbound
client messages (greeting / auth / CONNECT request); *when* the tunnel opens, and
whether a malformed server reply tears the connection down, is the proven
`Socks.hstep`'s decision — the same step the no-early-egress seam above is proven
over. So the deployment inherits `hstep_no_early_egress`: no application byte is
relayed until `hstep` reaches `established`.

The seam is total (any input, including a truncated frame or an unknown phase
byte, yields a defined 4-byte verdict) and allocation-bounded (one fixed 4-byte
output). It performs no recursion of its own; the underlying parsers
(`parseMethod` / `parseAuthStatus` / `parseReply` / `parseAddr`) are all
single-`match`, non-recursive, so the seam carries no unbounded-stack crash
class. -/

/-- Phase → wire byte. -/
def encodePhase : Socks.Phase → UInt8
  | .awaitGreeting => 0
  | .awaitAuth     => 1
  | .awaitReply    => 2
  | .established   => 3
...
/-- Wire byte → phase (total; an unknown byte is the safe terminal `failed`). -/
def decodePhaseByte : UInt8 → Socks.Phase
  | 0 => .awaitGreeting
  | 1 => .awaitAuth
  | 2 => .awaitReply
  | 3 => .established
...
/-- Egress action → wire byte. -/
def encodeOut : Socks.Out → UInt8
  | .wait        => 0
  | .sendAuth    => 1
  | .sendConnect => 2
  | .tunnelUp    => 3
...
/-- **The deployed SOCKS5 handshake decision, byte seam.** Input framing:
byte 0 = the current handshake phase (`decodePhaseByte`), byte 1 = the `hasAuth`
flag (`0` ⇒ no credentials), bytes 2.. = the server-response buffer accumulated
for that phase. It runs the **real** `Socks.hstep ⟨phase, hasAuth⟩ buf` and
returns exactly four bytes: `nextPhase :: outAction :: consumedHi :: consumedLo`,
where `consumed` (big-endian `u16`) is the byte prefix the phase's parser drained
on a complete parse (`0` while incomplete/errored, so the host keeps its
accumulation). The host reads `outAction` to decide its next move — `wait` (read
more), `sendAuth` / `sendConnect` (advance, drain `consumed`, send the next client
message), `tunnelUp` (open the relay), `closeErr` (bounded reject) — and never
opens the relay before `tunnelUp`. A frame shorter than two bytes is the defined
`failed`/`wait` verdict (the host reads more or drops). -/
@[export drorb_socks_step]
def socksStep (input : ByteArray) : ByteArray :=
  match input.toList with
  | phaseB :: authB :: buf =>
    let phase   := decodePhaseByte phaseB
    let hasAuth := authB != 0
    let r       := Socks.hstep ⟨phase, hasAuth⟩ buf
    let consumed := libConsumed phase buf
    ByteArray.mk #[encodePhase r.1.phase, encodeOut r.2,
      (consumed / 256).toUInt8, (consumed % 256).toUInt8]
  | _ => ByteArray.mk #[encodePhase .failed, encodeOut .wait, 0, 0]

/-- Method-select `05 00` in `awaitGreeting`: advance to `awaitReply`, send the
CONNECT request, drain the 2-byte greeting. -/
example : (socksStep ⟨#[0, 0, 0x05, 0x00]⟩).toList = [2, 2, 0, 2] := by
  simp (config := { decide := true }) [socksStep, Proto.Kernel.Shortcuts.ba_toList_eq]

/-- Method-select `05 02` in `awaitGreeting` WITH credentials: advance to
`awaitAuth`, send the auth sub-negotiation. -/
example : (socksStep ⟨#[0, 1, 0x05, 0x02]⟩).toList = [1, 1, 0, 2] := by
  simp (config := { decide := true }) [socksStep, Proto.Kernel.Shortcuts.ba_toList_eq]

/-- Method-select `05 02` with NO credentials: the handshake fails closed. -/
example : (socksStep ⟨#[0, 0, 0x05, 0x02]⟩).toList = [4, 4, 0, 2] := by
  simp (config := { decide := true }) [socksStep, Proto.Kernel.Shortcuts.ba_toList_eq]

/-- A success CONNECT reply `05 00 00 01 7f000001 0050` in `awaitReply`: the
tunnel opens (`established` / `tunnelUp`), draining the full 10-byte reply. -/
example : (socksStep ⟨#[2, 0, 0x05, 0x00, 0x00, 0x01, 0x7f, 0x00, 0x00, 0x01, 0x00, 0x50]⟩).toList
    = [3, 3, 0, 10] := by
  simp (config := { decide := true }) [socksStep, Proto.Kernel.Shortcuts.ba_toList_eq]

/-- A malformed reply (wrong version byte) in `awaitReply` is a BOUNDED reject:
`failed` / `closeErr`, nothing consumed — never a stuck state. -/
example : (socksStep ⟨#[2, 0, 0x06, 0x00, 0x00]⟩).toList = [4, 4, 0, 0] := by
  simp (config := { decide := true }) [socksStep, Proto.Kernel.Shortcuts.ba_toList_eq]

/-- A short buffer stutters `wait`, leaving the phase unchanged — the host reads
more bytes. -/
example : (socksStep ⟨#[0, 0, 0x05]⟩).toList = [0, 0, 0, 0] := by
  simp (config := { decide := true }) [socksStep, Proto.Kernel.Shortcuts.ba_toList_eq]

/-- A truncated frame (fewer than two header bytes) is the defined fail/wait
verdict — the seam is total. -/
example : (socksStep ⟨#[]⟩).toList = [4, 0, 0, 0] := by
  simp (config := { decide := true }) [socksStep, Proto.Kernel.Shortcuts.ba_toList_eq]
