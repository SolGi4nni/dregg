...
unless one is already present. At `v = altVal` it IS `stampAlt` — the deployed baked stamp. -/
def stampAltOf (v : Bytes) (hs : List (Bytes × Bytes)) : List (Bytes × Bytes) :=
  if hasAlt hs then hs else hs ++ [(altName, v)]

/-- **NO-REGRESSION (stamp).** The default value recovers `stampAlt`, byte-for-byte. -/
theorem stampAltOf_default (hs : List (Bytes × Bytes)) : stampAltOf altVal hs = stampAlt hs := rfl

/-- **The config-generalized stage** (operator VALUE `v`; `altVal` recovers `altStage`). -/
def altStageOf (v : Bytes) : Stage where
  name := "alt-svc"
  onRequest := fun c => .continue c
  onResponse := fun _ b => b.mapResp (fun r => { r with headers := stampAltOf v r.headers })

/-- **NO-REGRESSION anchor.** The default value recovers the deployed `altStage` — a `rfl` THEOREM. -/
theorem altStageOf_default : altStageOf altVal = altStage := rfl

/-- Status-stable for ANY value (safe to braid into a status-stable onion). -/
theorem altStageOf_statusStable (v : Bytes) : (altStageOf v).statusStable := fun _ _ => rfl

/-- **Operational (non-vacuous).** With value `v` the stage's byte-effect is `stampAltOf v` over
the finalized headers, so the emitted field VALUE is EXACTLY `v` — spec(cfg), for ANY tail/handler. -/
theorem altStageOf_effect (v : Bytes) (rest : List Stage) (h : Ctx → Response) (c : Ctx) :
    ((runPipeline (altStageOf v :: rest) h c).build).headers
      = stampAltOf v ((runPipeline rest h c).build).headers := by
  rw [pipeline_stage_effect (altStageOf v) rest h c c rfl]; rfl

#print axioms altStageOf_default
#print axioms altStageOf_effect
