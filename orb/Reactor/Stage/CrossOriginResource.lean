...
/-! ## Config-generalized: the stamp VALUE as data (Track 1 · Phase 2) -/

/-- **Config-generalized stamp.** Append the field with an operator-supplied VALUE `v`
unless one is already present. At `v = corpVal` it IS `stampCorp` — the deployed baked stamp. -/
def stampCorpOf (v : Bytes) (hs : List (Bytes × Bytes)) : List (Bytes × Bytes) :=
  if hasCorp hs then hs else hs ++ [(corpName, v)]

/-- **NO-REGRESSION (stamp).** The default value recovers `stampCorp`, byte-for-byte. -/
theorem stampCorpOf_default (hs : List (Bytes × Bytes)) : stampCorpOf corpVal hs = stampCorp hs := rfl

/-- **The config-generalized stage** (operator VALUE `v`; `corpVal` recovers `corpStage`). -/
def corpStageOf (v : Bytes) : Stage where
  name := "cross-origin-resource-policy"
  onRequest := fun c => .continue c
  onResponse := fun _ b => b.mapResp (fun r => { r with headers := stampCorpOf v r.headers })

/-- **NO-REGRESSION anchor.** The default value recovers the deployed `corpStage` — a `rfl` THEOREM. -/
theorem corpStageOf_default : corpStageOf corpVal = corpStage := rfl

/-- Status-stable for ANY value (safe to braid into a status-stable onion). -/
theorem corpStageOf_statusStable (v : Bytes) : (corpStageOf v).statusStable := fun _ _ => rfl

/-- **Operational (non-vacuous).** With value `v` the stage's byte-effect is `stampCorpOf v` over
the finalized headers, so the emitted field VALUE is EXACTLY `v` — spec(cfg), for ANY tail/handler. -/
theorem corpStageOf_effect (v : Bytes) (rest : List Stage) (h : Ctx → Response) (c : Ctx) :
    ((runPipeline (corpStageOf v :: rest) h c).build).headers
      = stampCorpOf v ((runPipeline rest h c).build).headers := by
  rw [pipeline_stage_effect (corpStageOf v) rest h c c rfl]; rfl

#print axioms corpStageOf_default
#print axioms corpStageOf_effect
