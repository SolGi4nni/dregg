...
unless one is already present. At `v = ppVal` it IS `stampPP` — the deployed baked stamp. -/
def stampPPOf (v : Bytes) (hs : List (Bytes × Bytes)) : List (Bytes × Bytes) :=
  if hasPP hs then hs else hs ++ [(ppName, v)]

/-- **NO-REGRESSION (stamp).** The default value recovers `stampPP`, byte-for-byte. -/
theorem stampPPOf_default (hs : List (Bytes × Bytes)) : stampPPOf ppVal hs = stampPP hs := rfl

/-- **The config-generalized stage** (operator VALUE `v`; `ppVal` recovers `ppStage`). -/
def ppStageOf (v : Bytes) : Stage where
  name := "permissions-policy"
  onRequest := fun c => .continue c
  onResponse := fun _ b => b.mapResp (fun r => { r with headers := stampPPOf v r.headers })

/-- **NO-REGRESSION anchor.** The default value recovers the deployed `ppStage` — a `rfl` THEOREM. -/
theorem ppStageOf_default : ppStageOf ppVal = ppStage := rfl

/-- Status-stable for ANY value (safe to braid into a status-stable onion). -/
theorem ppStageOf_statusStable (v : Bytes) : (ppStageOf v).statusStable := fun _ _ => rfl

/-- **Operational (non-vacuous).** With value `v` the stage's byte-effect is `stampPPOf v` over
the finalized headers, so the emitted field VALUE is EXACTLY `v` — spec(cfg), for ANY tail/handler. -/
theorem ppStageOf_effect (v : Bytes) (rest : List Stage) (h : Ctx → Response) (c : Ctx) :
    ((runPipeline (ppStageOf v :: rest) h c).build).headers
      = stampPPOf v ((runPipeline rest h c).build).headers := by
  rw [pipeline_stage_effect (ppStageOf v) rest h c c rfl]; rfl

#print axioms ppStageOf_default
#print axioms ppStageOf_effect
