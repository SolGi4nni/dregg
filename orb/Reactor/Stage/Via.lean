...
unless one is already present. At `v = viaVal` it IS `stampVia` — the deployed baked stamp. -/
def stampViaOf (v : Bytes) (hs : List (Bytes × Bytes)) : List (Bytes × Bytes) :=
  if hasVia hs then hs else hs ++ [(viaName, v)]

/-- **NO-REGRESSION (stamp).** The default value recovers `stampVia`, byte-for-byte. -/
theorem stampViaOf_default (hs : List (Bytes × Bytes)) : stampViaOf viaVal hs = stampVia hs := rfl

/-- **The config-generalized stage** (operator VALUE `v`; `viaVal` recovers `viaStage`). -/
def viaStageOf (v : Bytes) : Stage where
  name := "via"
  onRequest := fun c => .continue c
  onResponse := fun _ b => b.mapResp (fun r => { r with headers := stampViaOf v r.headers })

/-- **NO-REGRESSION anchor.** The default value recovers the deployed `viaStage` — a `rfl` THEOREM. -/
theorem viaStageOf_default : viaStageOf viaVal = viaStage := rfl

/-- Status-stable for ANY value (safe to braid into a status-stable onion). -/
theorem viaStageOf_statusStable (v : Bytes) : (viaStageOf v).statusStable := fun _ _ => rfl

/-- **Operational (non-vacuous).** With value `v` the stage's byte-effect is `stampViaOf v` over
the finalized headers, so the emitted field VALUE is EXACTLY `v` — spec(cfg), for ANY tail/handler. -/
theorem viaStageOf_effect (v : Bytes) (rest : List Stage) (h : Ctx → Response) (c : Ctx) :
    ((runPipeline (viaStageOf v :: rest) h c).build).headers
      = stampViaOf v ((runPipeline rest h c).build).headers := by
  rw [pipeline_stage_effect (viaStageOf v) rest h c c rfl]; rfl

#print axioms viaStageOf_default
#print axioms viaStageOf_effect
