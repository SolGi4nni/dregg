...
/-- `String.intercalate` on a singleton list is the sole element. The public
replacement for unfolding the (now hygienic) `String.intercalate.go` on a
one-directive list. -/
theorem intercalate_one (s a : String) : String.intercalate s [a] = a := rfl

/-- `String.intercalate` on a two-element list. Public replacement for the
hygienic `String.intercalate.go` on a two-directive list. -/
theorem intercalate_two (s a b : String) :
    String.intercalate s [a, b] = a ++ s ++ b := rfl

/-- `String.intercalate` on a three-element list. Public replacement for the
hygienic `String.intercalate.go` on a three-directive list. -/
theorem intercalate_three (s a b c : String) :
    String.intercalate s [a, b, c] = a ++ s ++ b ++ s ++ c := rfl
...
theorem hstsRender_matches_spec (h : Hsts) :
  ...
  cases hi : h.includeSubDomains <;> cases hp : h.preload <;>
    (simp only [hstsRender, specHstsValue, specHstsDirectives, hi, hp,
      ...
      intercalate_one, intercalate_two, intercalate_three]
     <;> apply String.ext
