...
theorem find?_first {α} (p : α → Bool) (pre : List α) (c : α) (post : List α)
  ...
  | cons a pre' ih =>
    have ha : p a = false := hpre a (List.mem_cons_self)
...
def parseETagList (s : String) : List ETag :=
  (s.splitOn ",").filterMap (fun raw =>
    ...
    if t == "" || t == "*" then none
      ...
      let (weak, rest) := if "W/".isPrefixOf t then (true, (t.drop 2).toString) else (false, t)
      let tag := (rest.dropWhile (· == '"')).toString.dropRightWhile (· == '"')
      some ⟨weak, tag⟩)
