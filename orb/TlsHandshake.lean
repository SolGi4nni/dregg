...
/-- Select the ALPN protocol: the first name the client offered that this
DEPLOYMENT supports, in the deployment's preference order. `supported` is a
parameter rather than the global `serverAlpn` because a front may serve a
protocol that cannot carry every application: the ts2021 control front completes
its handshake with an HTTP/1.1 `101 Switching Protocols`, and HTTP/2 has no
protocol-switch mechanism at all (RFC 9113 §8.1 drops `101`), so that front pins
`[alpnHttp11]` while the general HTTPS front keeps the full `serverAlpn`. -/
def negotiateAlpn (supported offered : List Tls.Bytes) : Option Tls.Bytes :=
  supported.find? (fun p => offered.contains p)

/-- **ALPN soundness**: a selected protocol was offered by the client and is
supported by this deployment. -/
theorem negotiateAlpn_sound {supported offered : List Tls.Bytes} {p : Tls.Bytes}
    (h : negotiateAlpn supported offered = some p) :
    p ∈ supported ∧ p ∈ offered := by
  refine ⟨List.mem_of_find?_eq_some h, ?_⟩
  have hp := List.find?_some h
  simpa using hp
...
def chooseAlpn (supported : List Tls.Bytes) : Option (List Tls.Bytes) → AlpnChoice
  | none => .ok none
  | some offered =>
    match negotiateAlpn supported offered with
    | some p => .ok (some p)
    | none => .reject
...
structure ServerParams where
  ...
  /-- **The application protocols THIS front offers**, in preference order
  (RFC 7301). Defaults to the full `serverAlpn` (`h2` then `http/1.1`), so every
  existing deployment is byte-for-byte unchanged. A front whose application
  cannot run over a protocol must narrow this: the ts2021 control front pins
  `[alpnHttp11]` because its handshake is an HTTP/1.1 `101 Switching Protocols`
  upgrade and HTTP/2 has no protocol switch. -/
  alpnSupported : List Tls.Bytes := serverAlpn
...
/-- **0-RTT is gated OFF unless the deployment provisions it.** When the
deployment stands up no anti-replay register — the default: the
`DRORB_TLS_EARLY_DIR` opt-in is absent, so `earlyDataOk` is the constant
`false` — the 0-RTT acceptance gate is closed for EVERY ClientHello,
regardless of retry, suite, ALPN, or offered PSK/ticket. This is the exact
property the conformance battery's G-family asserts but cannot observe on the
wire (EncryptedExtensions is encrypted): absent the opt-in, no early-data offer
is ever admitted. -/
theorem earlyGate_closed_when_unprovisioned (params : ServerParams)
    (retried : Option Retry) (ch : ClientHello) (suite : Nat)
    (alpnSel : Option Tls.Bytes) (info? : Option TicketInfo)
    (hoff : ∀ id, params.earlyDataOk id = false) :
    earlyGate params retried ch suite alpnSel info? = false := by
  unfold earlyGate
  cases hpsk : ch.psk with
  | none => simp [hpsk]
  | some op => simp [hpsk, hoff op.identity]
        ...
        if (retried.map (·.suite)).getD suite ≠ suite then rejectWith illegalParameterDesc
          ...
          | some _sa =>
            ...
            -- The extension itself is mandatory (§9.2, `missing_extension`).
              match chooseAlpn params.alpnSupported ch.alpnOffered with
...
/-- **A flight built with no accepted early secret never enters the early
phase** (the unconditional companion of `buildFlight_early`): at `early? = none`
the `Established` a successful flight carries has `earlyAccepted = false` — the
PSK arm sets `earlyAccepted := none.isSome`, the full-handshake arm defaults it
`false`. So the only source of an accepted early phase is a caller who supplied
`early?.isSome`, which in `kexStep` rides solely behind `earlyGate`. -/
theorem buildFlight_no_early (params : ServerParams) (suite : Nat)
    (alpnSel : Option Tls.Bytes) (psk? : Option ByteArray) (group : Nat)
    (sid : Tls.Bytes) (pub dhe t0 : ByteArray) (entry : CertEntry)
    (statusReq : Bool) (est : Established) (fl : ByteArray) (hp : Tls.Bytes)
    (h : buildFlight params suite alpnSel psk? group sid pub dhe t0
           entry statusReq none = some (est, fl, hp)) :
    est.earlyAccepted = false := by
  simp only [buildFlight] at h
  repeat' split at h
  all_goals first
    | (simp only [Option.some.injEq, Prod.mk.injEq] at h
...
/-- **No session accepts 0-RTT when the deployment does not provision it.**
Under the default `earlyDataOk = fun _ => false` (`DRORB_TLS_EARLY_DIR` unset),
any `Established` that `kexStep` produces has `earlyAccepted = false`:
`earlyGate_closed_when_unprovisioned` forces the local `early?` to `none`, and
every flight-building arm then goes through `buildFlight_no_early`. So no early
traffic secret is derived, no early channel opens, and — additively, without
touching the hybrid-KEX downgrade guarantees — the pragmatic default deployment
is provably 0-RTT-free. -/
theorem kexStep_no_early_when_unprovisioned (params : ServerParams)
    (retried : Option Retry) (ch : ClientHello) (suite : Nat)
    (alpnSel : Option Tls.Bytes) (buf : Tls.Bytes) (est : Established)
    (hoff : ∀ id, params.earlyDataOk id = false)
    (hres : (kexStep params retried ch suite alpnSel buf).1 = .waitClientFinished est) :
    est.earlyAccepted = false := by
  have hg : ∀ i, earlyGate params retried ch suite alpnSel i = false :=
    fun i => earlyGate_closed_when_unprovisioned params retried ch suite alpnSel i hoff
  unfold kexStep at hres
  simp only [hg, Bool.false_eq_true, if_false] at hres
  repeat' split at hres
  all_goals first
    | (exact absurd hres (fun h => HsState.noConfusion h))
    | (injection hres with hst
       have hbf := buildFlight_no_early _ _ _ _ _ _ _ _ _ _ _ _ _ _ (by assumption)
       rw [← hst]
       exact hbf)

/-- **The early-data CHANNEL stays empty for a 0-RTT-free session.** A
`serverStep` from a `waitClientFinished` whose `Established` did not accept early
data emits nothing on the `earlyData` channel — the contrapositive of
`earlyData_only_when_accepted`. -/
theorem serverStep_no_early_from_established (params : ServerParams)
    (est : Established) (buf : Tls.Bytes)
    (he : est.earlyAccepted = false) :
    (serverStep params (.waitClientFinished est) buf).2.earlyData = [] := by
  rcases hx : (serverStep params (.waitClientFinished est) buf).2.earlyData with _ | ⟨a, l⟩
  · rfl
  · exfalso
    obtain ⟨est', hst, hacc⟩ :=
      earlyData_only_when_accepted params (.waitClientFinished est) buf (by rw [hx]; simp)
    injection hst with h
    subst h
    rw [he] at hacc
    exact absurd hacc (by simp)

/-- **End-to-end: the default deployment never emits 0-RTT early data.**
Composing the two facts above: a session `kexStep` established under the
unprovisioned default (`earlyDataOk = fun _ => false`) carries no early channel,
so any subsequent `serverStep` on it delivers `earlyData = []`. This is the
proof-level closure of the battery's G-family — what the encrypted flight hides
from the wire probe, the semantics settle. -/
theorem no_early_data_when_unprovisioned (params : ServerParams)
    (retried : Option Retry) (ch : ClientHello) (suite : Nat)
    (alpnSel : Option Tls.Bytes) (buf buf2 : Tls.Bytes) (est : Established)
    (hoff : ∀ id, params.earlyDataOk id = false)
    (hres : (kexStep params retried ch suite alpnSel buf).1 = .waitClientFinished est) :
    (serverStep params (.waitClientFinished est) buf2).2.earlyData = [] :=
  serverStep_no_early_from_established params est buf2
    (kexStep_no_early_when_unprovisioned params retried ch suite alpnSel buf est hoff hres)
...
theorem data_append (a b : ByteArray) : (a ++ b).data = a.data ++ b.data := by
  ...
  simp [ByteArray.append, ByteArray.copySlice, ByteArray.size,
        Array.extract_empty_of_size_le_start (xs := a.data) (Nat.le_add_right _ _)]
...
theorem ticket_roundtrip (tkey nonce psk t : ByteArray) (hn : nonce.size = 12)
  ...
  | some ct =>
    ...
    have hlen : nonce.data.toList.length = 12 := by
      rw [Array.length_toList]; exact hn
...
#print axioms TlsHandshake.earlyGate_closed_when_unprovisioned
#print axioms TlsHandshake.buildFlight_no_early
#print axioms TlsHandshake.kexStep_no_early_when_unprovisioned
#print axioms TlsHandshake.serverStep_no_early_from_established
#print axioms TlsHandshake.no_early_data_when_unprovisioned
