...
theorem accept_matches_step (cap : Nat) (m : OpenMsg) (f : Frame)
  ...
  case continuation.idle => exact Verdict.noConfusion h
  -- text, binary / open message: discipline-rejected
  case text.text => exact Verdict.noConfusion h
  case text.binary => exact Verdict.noConfusion h
  case binary.text => exact Verdict.noConfusion h
  case binary.binary => exact Verdict.noConfusion h
