...
# Default `auto` = the reactor the shipped binary picks (see launch.sh).
SUT_IO="${SUT_IO:-auto}"
...
# multi.py reads $SUT_PORT/$CADDY_PORT/$H2O_PORT as its defaults — same knob as
# this launcher. (Note the two lanes use DIFFERENT default ports: driving the
# multi lane with diff.py's defaults hits nothing at all.)
echo "run the differ: SUT_PORT=$SUT_PORT CADDY_PORT=$CADDY_PORT H2O_PORT=$H2O_PORT python3 $HERE/multi.py"
