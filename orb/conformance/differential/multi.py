...
lanes stay in lock-step — extending one corpus extends both. It reuses diff.py's
reachability guards for the same reason: three dead ports "agree" on every one
of the 63 requests, which this used to report as `clean (all 3 agree): 63`.

  * liveness precondition — sut, caddy and h2o must each answer a known-good
    probe with a parseable HTTP status line before any comparison runs;
  * transport faults (connect/send) mid-run invalidate the run;
  * a request where every server failed is scored `all-error`, never clean.

Ports come from CLI flags, then $SUT_PORT/$CADDY_PORT/$H2O_PORT (what
launch_multi.sh exports), then the built-in defaults.
...
def main():
    ...
    ap.add_argument("--sut-port", type=int, default=diff.env_port("SUT_PORT", 18950),
                    help="default: $SUT_PORT, else 18950")
    ap.add_argument("--caddy-port", type=int, default=diff.env_port("CADDY_PORT", 18951),
                    help="default: $CADDY_PORT, else 18951")
    ap.add_argument("--h2o-port", type=int, default=diff.env_port("H2O_PORT", 18952),
                    help="default: $H2O_PORT, else 18952")
    ...
    # --- precondition: all three endpoints are live HTTP servers ----------
    # Three unreachable ports agree on everything. Exits 2 if any is dead.
    diff.assert_live(args.host, [(n, ports[n]) for n in SERVERS])
    ...
    for cid, cat, desc, raw in corpus:
        ...
        for name in SERVERS:
            ...
            errs[name] = err
            if diff.is_transport_fault(err):
                faults.append({"id": cid, "side": name, "error": err})
            parsed = err if err else diff.parse_response(rawb)
            resp[name] = parsed
            dd[name] = dims(parsed)

        # Every server errored: no response was compared anywhere. That is the
        # false-green shape (identical failures look like unanimity) and is
        # never counted as clean.
        all_error = all(errs[n] for n in SERVERS)
    ...
    if n_all_error:
        print(f"  ALL-ERROR (no response anywhere; NOT agreement) : {n_all_error}")
    ...
    invalid = bool(faults) or bool(n_all_error)
    if invalid:
        print("")
        print("=" * 72)
        if faults:
            ...
            for f in faults[:10]:
                print(f"  {f['side']:6s} {f['id']}: {f['error']}")
            if len(faults) > 10:
                print(f"  ... and {len(faults) - 10} more")
        if n_all_error:
            print(f"FATAL: {n_all_error} request(s) got NO response from any server.")
    ...
    if args.json:
        with open(args.json, "w") as f:
            json.dump({"summary": {"requests": len(corpus), "clean": n_clean,
                                   "all_error": n_all_error, "invalid": invalid,
