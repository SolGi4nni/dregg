...
# `auto` — the SAME value the shipped binary defaults to (`IoMode::Auto`, see
# crates/dataplane/src/main.rs), so the gate attests the reactor production
# actually runs: io_uring on Linux, the kqueue reactor on macOS/BSD.
#
# It used to default to `blocking`, the portable thread-per-connection
# fallback, which meant every number this suite printed described a reactor
# nobody deploys. That is how the WebSocket gap survived: the upgrade fork was
# only wired on the portable path, so ~190 proven Ws/ theorems and a 514/517
# Autobahn run were unreachable on the default reactor and the harness only
# ever tested the one reactor where they worked.
#
# `DRORB_CONF_IO=blocking` still selects the portable fallback explicitly (it
# stays a first-class, runnable target — the point is that it is no longer the
# ONLY thing tested); `uring` / `kqueue` pin one reactor regardless of host.
# The braided-dataplane scenario inherits the same mode so both instances match.
CONF_IO = os.environ.get("DRORB_CONF_IO", "auto")
