...
#   DUAL_IO         reactor io backend for the serve. Default `auto` = the mode
#                   the shipped binary picks (io_uring on Linux, kqueue on
#                   macOS/BSD), so the A/B runs on the deployed reactor. Set
#                   DUAL_IO=blocking for the portable fallback.
