...
Env:
    ...
    DRORB_LEAK_IO     --io reactor for a serve this scan launches itself
                      (default `auto` = the mode the shipped binary picks:
                      io_uring on Linux, kqueue on macOS/BSD). Was hardcoded
                      `blocking`, so a standalone scan cleared a reactor
                      nobody deploys. Set DRORB_LEAK_IO=blocking for the
                      portable fallback. In --target mode the reactor is
                      whatever the already-running serve was launched with.
...
def launch_serve():
    ...
    proc = subprocess.Popen(
        [DATAPLANE, "--bind", f"{HOST}:{PORT}", "--no-udp", "--io",
         os.environ.get("DRORB_LEAK_IO", "auto")],
