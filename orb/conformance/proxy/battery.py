...
class Serve:
    def __init__(self, serve_bin, bind_port, backends, logpath):
        ...
        # WHICH proxy lane this battery drives. Default: the LEGACY proxy_hook
        # lane (effect seam OFF) it was written against. PROXY_SEAM=1 drives the
        # effect/continuation seam instead (`step` yielding proxyDial), which is
        # the lane the io_uring reactor implements natively.
        #
        # FINDING (2026-07-24): these two lanes are NOT reactor-equivalent. The
        # proxy_hook fork lives only in crates/dataplane/src/blocking.rs:474
        # (`!interp::enabled() && proxy_hook::is_proxy_path`); uring.rs has no
        # counterpart, so with the seam OFF a /api request on io_uring falls
        # through to the route table and 404s instead of forwarding. This
        # battery pinned `--io blocking` AND unset the seam, which is exactly
        # why that gap stayed invisible.
        if os.environ.get("PROXY_SEAM") == "1":
            env["DRORB_EFFECT_SEAM"] = "1"
        else:
            env.pop("DRORB_EFFECT_SEAM", None)
        self.log = open(logpath, "wb")
        # The `--io` reactor for the proxied serve. Default `auto` = the mode
        # the shipped binary picks (io_uring on Linux, kqueue on macOS/BSD), so
        # this battery grades the reactor production runs. It used to be a
        # hardcoded `blocking` on the claim that "the uring/kqueue fast paths
        # serve /api locally"; that pin is stale — the forwarding fork IS
        # reachable on io_uring (driver.py handler-proxy-forward and every
        # fabric-* scenario serve real X-Backend responses under --io auto).
        # PROXY_IO=blocking still selects the portable fallback explicitly.
        self.p = subprocess.Popen(
            [serve_bin, "--bind", "127.0.0.1:%d" % bind_port, "--no-udp",
