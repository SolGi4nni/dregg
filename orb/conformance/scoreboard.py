...
Registered suites (run when present, reported PENDING when not landed yet):
  ...
  leak       conformance/leak_scan.py             info-disclosure leak-scan (also a
                                                  HARD gate in run.sh)
...
Usage:
    python3 conformance/scoreboard.py                 # deployed reactor (auto)
    CONF_IO=blocking python3 conformance/scoreboard.py # portable fallback
    CONF_SCOREBOARD_PORT=18906 CONF_IO=uring \
...
# The `--io` reactor the scored serve runs. Default `auto` = whatever the
# shipped binary picks (IoMode::Auto -> io_uring on Linux, kqueue on macOS), so
# the scoreboard grades the reactor production actually serves on. `blocking`
# (the portable thread-per-connection fallback) stays explicitly selectable via
# CONF_IO; it just no longer silently owns the gate.
IO = os.environ.get("CONF_IO", "auto")
...
def run_leak_suite(serve):
    """Information-disclosure leak-scan (conformance/leak_scan.py) as a scored suite.

    The leak-scan owns its own `--json` shape (routes with a per-class `leaks`
    map), not the `checks` list every other suite writes, so it needs its own
    adapter. It is driven with `--target` against THIS scoreboard's dedicated
    serve (never launching a second one), and each route becomes one graded
    check: PASS when the route disclosed nothing, FAIL (itemizing the leaked
    classes) when it did. In run.sh the same scan is a HARD gate (nonzero exit
    fails CI); here it is scored alongside the RFC suites for visibility.
    """
    sid = "leak"
    title = "Information-disclosure leak-scan (25 routes x 4 leak-classes)"
    script = os.path.join(HERE, "leak_scan.py")
    if not os.path.exists(script):
        return {"id": sid, "title": title, "status": "PENDING",
                "pass": 0, "fail": 0, "skip": 0, "checks": [],
                "note": "leak_scan.py not landed yet"}
    if not serve.ensure():
        return {"id": sid, "title": title, "status": "ERROR",
                "pass": 0, "fail": 0, "skip": 0, "checks": [],
                "note": "serve could not be (re)launched for the leak-scan"}
    out_json = os.path.join(HERE, f"results_leak-{PORT}.json")
    try:
        subprocess.run([PYTHON, script, "--target", f"{HOST}:{PORT}",
                        "--json", out_json], cwd=HERE, env=dict(os.environ),
                       capture_output=True, text=True, timeout=300)
    except subprocess.TimeoutExpired:
        return {"id": sid, "title": title, "status": "ERROR",
                "pass": 0, "fail": 0, "skip": 0, "checks": [],
                "note": "leak-scan timed out (300s)"}
    try:
        ...
        return {"id": sid, "title": title, "status": "ERROR",
                "pass": 0, "fail": 0, "skip": 0, "checks": [],
                "note": "leak-scan wrote no parseable results json"}
    checks = []
    for r in d.get("results", []):
        ...
        if leaks:
            ev = "; ".join(f"{cid}: {e}"
                           for cid, items in leaks.items() for e in items)
            checks.append({"id": route, "verdict": "FAIL",
                           "criterion": f"[leak] {desc}", "observed": ev[:300]})
        elif r.get("reachable"):
            checks.append({"id": route, "verdict": "PASS",
                           "criterion": f"[leak] {desc}", "observed": ""})
        else:
            checks.append({"id": route, "verdict": "SKIP",
                           "criterion": f"[leak] {desc}",
                           "observed": "route unreachable"})
    return {"id": sid, "title": title, "status": "RAN",
            "pass": sum(1 for c in checks if c["verdict"] == "PASS"),
            "fail": sum(1 for c in checks if c["verdict"] == "FAIL"),
            "skip": sum(1 for c in checks if c["verdict"] not in ("PASS", "FAIL")),
            "checks": checks, "note": f"verdict={d.get('verdict', '?')}"}
...
def main():
    ...
    try:
        ...
        print("[scoreboard] leak (information-disclosure) ...")
        serve.ensure()
        suites.append(run_leak_suite(serve))
