...
def run_battery(serve_port, dead_serve_port):
    ...
    # --- (d1) 502 on upstream connect-fail -------------------------------
    resp, err = send_raw(dead_serve_port, b"GET /api/echo HTTP/1.1\r\nHost: t\r\n\r\n")
    st = status_of(resp) if resp else None
    record("d1", "502 Bad Gateway on upstream connect-fail", "9110 15.6.3",
...
def main():
    ...
    default_bin = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "target/release/dataplane")
