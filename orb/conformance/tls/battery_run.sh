...
# conformance/tls/, waits for readiness, runs battery.py my-hand against it,
...
# DEPRECATED — POSTURE-MISMATCH RUNNER. This runner launched the serve in the
# serve's DEFAULT `preferred` KEX posture, but the TLS 1.3 battery it drove
# asserts the HYBRID PIN (`required`). That mismatch produced FALSE fails on the
# pin checks: a classical-only client the battery expected to be REFUSED was
# instead honored by the preferred-posture serve. The canonical runner pins the
# posture it asserts (DRORB_TLS_KEX=required) — conformance/tls/tls13_battery_run.sh.
# Forward there so the broken posture can never be re-run by accident; the stale
# body below is kept for history and is unreachable past this exec.
_HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
echo "note: tls/battery_run.sh is deprecated -> tls13_battery_run.sh (posture-correct)" >&2
exec "$_HERE/tls13_battery_run.sh" "$@"
...
# UNREACHABLE (see the exec forward above); kept tracking TLS_IO so the io
# default guard does not have to carry an exemption for dead code.
TLS_IO="${TLS_IO:-auto}"
