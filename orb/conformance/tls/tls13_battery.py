...
class Battery:
    ...
    def check_G_early_data(self):
        # 0-RTT is gated on the DRORB_TLS_EARLY_DIR opt-in (an anti-replay
        # register); the battery runs the serve WITHOUT it, so the deployment is
        # unprovisioned and 0-RTT must never be accepted. EncryptedExtensions —
        # where an early_data ACCEPT would show — is encrypted, and the
        # subprocess client performs no resumption, so the accept path is not
        # observable raw. What IS observable: an early_data offer never coaxes a
        # 0-RTT ServerHello out of the pinned server. We drive BOTH offer shapes
        # and require neither to complete a 1.3 handshake:
        #   (a) early_data + NO key_share  -> §4.1.4 HelloRetryRequest(0x11EC):
        #       the 0-RTT offer does not bypass the hybrid pin, and no early
        #       ServerHello is emitted (the client must still complete the pinned
        #       KEX first — 0-RTT cannot even begin here).
        #   (b) early_data + classical share only -> handshake_failure: rejected.
        # The unobservable remainder (the encrypted EE never carries early_data,
        # no early channel opens) is CLOSED by proof: TlsHandshake.lean's
        # `no_early_data_when_unprovisioned` (axioms {propext, Quot.sound}) shows
        # every session kexStep establishes under earlyDataOk=fun _=>false emits
        # earlyData = [] — the deployment default this battery exercises.
        hello_noshare = build_client_hello(
            supported_groups=[GRP_X25519MLKEM768, GRP_X25519],
            key_shares=[],
            add_early_data=True,
        )
        buf_a = raw_probe(self.host, self.port, hello_noshare)
        sh_a = first_handshake_serverhello(buf_a)
        alert_a = first_alert(buf_a)
        completed_a = sh_a is not None and not sh_a["is_hrr"] \
            and sh_a["selected_version"] == TLS13
        hrr_a = sh_a is not None and sh_a["is_hrr"] \
            and sh_a["key_share_group"] == GRP_X25519MLKEM768

        hello_classical = build_client_hello(
            supported_groups=[GRP_X25519MLKEM768, GRP_X25519],
            key_shares=[(GRP_X25519, b"\x55" * 32)],
            add_early_data=True,
        )
        buf_b = raw_probe(self.host, self.port, hello_classical)
        sh_b = first_handshake_serverhello(buf_b)
        alert_b = first_alert(buf_b)
        completed_b = sh_b is not None and not sh_b["is_hrr"] \
            and sh_b["selected_version"] == TLS13

        # PASS: neither early_data offer completes a 1.3 handshake, the no-share
        # form is answered by HRR(0x11EC), and the classical form is refused —
        # 0-RTT is not accepted on the wire, and the encrypted remainder is
        # closed by the machine-checked no_early_data_when_unprovisioned.
        ok = (not completed_a) and hrr_a and (not completed_b) \
            and (alert_b is not None or buf_b == b"" or buf_b.startswith(b"ERR:"))
        self.add("G1", "G early-data",
                 "early_data offer is not accepted on an unprovisioned deployment "
                 "(0-RTT gated; encrypted remainder proven)",
                 "PASS" if ok else "FAIL",
                 "no-share->%s classical->%s; encrypted-EE closed by "
                 "no_early_data_when_unprovisioned" %
                 (("HRR(0x11EC)" if hrr_a else
                   ("SH" if completed_a else ("alert=%s" % (alert_a and alert_a["name"])))),
                  ("alert=%s" % (alert_b and alert_b["name"])) if alert_b else
                  ("SH" if completed_b else "closed")))
