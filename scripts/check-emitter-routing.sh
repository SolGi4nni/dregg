...
declare -A DIAGNOSED=(
  ...
  [EmitKimchiPoseidon2.lean]="⚑ WORST CASE IN THIS SET, AND NOT A STALE ARTIFACT — AN ABSENT ONE. bridge/mina-zkapp/src/Poseidon2Native.ts:107 resolves src/generated/kimchi-poseidon2-w16.json at RUNTIME; that file exists NOWHERE in the tree, and bridge/mina-zkapp/npm-scripts-allow.tsv:36 records the poseidon2-native gate as DISABLED *because* it is missing. Worse: this emitter does not currently RUN — \`lake env lean --run EmitKimchiPoseidon2.lean\` dies with 'deep recursion was detected at interpreter' (measured 2026-07-31, warm oleans). A disabled gate whose precondition is an artifact nobody can emit."
