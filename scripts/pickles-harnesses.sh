...
HARNESSES=(
  ...
  "stepmain|pickles-stepmain-harness|8|step_verifier.verify_one ASSEMBLED: a copy-wired Poseidon sponge, chained EndoMulScalar tied to the squeeze, an MSM whose scalar IS the derived challenge, endo_mul fold rounds, deferred b(zeta)+combined_inner_product, ft_eval0 + Plonk_checks.checked + the six-gate linearization constant term, the 43-column evaluation absorption with opt-sponge masking, PRIMARY_LEN 67 - all seven gate types in one circuit"
)
