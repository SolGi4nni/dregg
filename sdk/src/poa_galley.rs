...
//!
//! This module is only the signed intent carrier in front of the Lean Galley
//! runtime.  The server issues an opaque action token bound to the deployment,
//! daily, predecessor head, next sequence, allowed action constraints, and
//! expiry.  None of those authority-bearing values can be supplied separately
//! by a player.  The server must consume the token atomically with the accepted
//! Galley event.
//!
//! Player identity is likewise absent from the event.  The builder derives the
//! agent cell from the signing public key, while the signed-turn decoder verifies
//! the outer Ed25519 signature and binds `SignedTurn.signer` to that cell.  Action
//! authorization bytes are signatures, not an identity source.
//!
//! `openMaintenance` is deliberately not a player command.  Its reserved tag is
//! recognized and refused rather than falling through to an ordinary event.
//! Holder sponsorship carries exactly one short-lived server capability receipt
//! id; it never carries a token balance, weight, slot, or caller-authored grant.
//! That public id must be one-use and context-bound.  Accepted history retains
//! only its nullifier/certificate digest, never the receipt id as a reusable
//! status or deanonymization handle.
...
/// The id must not appear in unauthenticated status responses or finalized
/// public views; those retain only a one-way nullifier/certificate digest.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
///
/// All identity and hidden state authority is deliberately absent.  Content and
/// choice ids are exact signed intent, but the action token constrains them
/// against the server/Lean-authored state before execution.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
pub enum GalleyCommandError {
    #[error("reserved Galley command marker is malformed: {0}")]
    MalformedReserved(&'static str),
    #[error("Galley open-maintenance is operator-only and cannot be player-authored")]
    OperatorOnlyOpenMaintenance,
    #[error("unknown reserved Galley command tag {0}")]
...
pub enum GalleyCarrierError {
    #[error("invalid Galley player carrier: {0}")]
    InvalidShape(&'static str),
    #[error(transparent)]
    Command(#[from] GalleyCommandError),
}
...
/// This deliberately reuses the already-deployed PoA/default-agent derivation
/// rather than creating a Galley-specific identity domain.
pub fn galley_player_cell(signer_public_key: &[u8; 32]) -> CellId {
    crate::poa_signal::signal_player_cell(signer_public_key)
}

/// Construct the only player-authored turn shape accepted by this client.
///
...
pub fn galley_player_command_turn(
    ...
    let action = crate::raw::unsigned_action_named(agent, GALLEY_COMMAND_METHOD_V1, vec![effect]);
...
fn galley_command_hybrid_fee(turn: &dregg_turn::Turn) -> u64 {
...
mod tests {
    ...
    #[test]
    fn every_reserved_marker_failure_refuses_instead_of_falling_through() {
    ...
    #[test]
    fn noncanonical_tags_and_choices_refuse() {
    ...
    #[test]
    fn holder_receipt_is_exactly_one_lane_and_forbidden_on_other_tags() {
        let reserved = symbol(GALLEY_COMMAND_TOPIC_V1);
        let sponsor_without_receipt = Event::new(
            reserved,
            vec![
                field_from_u64(HOLDER_SPONSORSHIP_TAG_V1),
                action_token().to_bytes(),
        ...
        assert!(matches!(
            classify_galley_event(&sponsor_without_receipt),
            Err(GalleyCommandError::MalformedReserved(_))
        ));

        let mut receipt_on_vote = galley_command_event(commands()[0]);
        receipt_on_vote.data.push(holder_receipt_id().to_bytes());
        assert!(matches!(
            classify_galley_event(&receipt_on_vote),
            Err(GalleyCommandError::MalformedReserved(_))
        ));
    ...
    #[test]
    fn exact_carrier_has_one_root_one_event_and_signer_derived_agent() {
        for command in commands() {
            ...
            let dregg_turn::Effect::EmitEvent { cell, .. } = &action.effects[0] else {
                panic!("exact Galley carrier must emit one event")
            };
    ...
    #[test]
    fn exact_carrier_refuses_piggyback_effects_roots_and_metadata() {
    ...
    #[test]
    fn exact_carrier_refuses_emitter_substitution() {
    ...
    fn signed_envelope_is_the_only_player_identity_source() {
        let command = commands()[0];
        let signing_key = dregg_types::SigningKey::from_bytes(&[0x71; 32]);
        let signer = signing_key.public_key();
        let turn = galley_player_command_turn(&signer.0, 4, None, command);
        let signature = dregg_types::sign(&signing_key, &turn.hash());
        let signed = crate::SignedTurn {
            turn,
            signature,
            signer,
            pq_signature: vec![],
        ...
        let wrong_agent_turn = galley_player_command_turn(&[0x72; 32], 4, None, command);
        let wrong_agent_signature = dregg_types::sign(&signing_key, &wrong_agent_turn.hash());
        let wrong_agent = crate::SignedTurn {
            turn: wrong_agent_turn,
            signature: wrong_agent_signature,
            signer,
            pq_signature: vec![],
            pq_signer: vec![],
        };
        assert!(matches!(
            command_from_exact_galley_signed_turn(&wrong_agent),
            Err(GalleyCarrierError::InvalidShape(
                "outer signer does not derive the Galley player agent"
            ))
        ));
        ...
        invalid_signature.signature = dregg_types::Signature([0; 64]);
        assert!(matches!(
    ...
    #[test]
    fn action_signature_bytes_are_not_misread_as_player_key() {
        ...
        let mut turn = galley_player_command_turn(&SIGNER, 3, None, command);
    ...
    #[test]
    fn operator_open_maintenance_cannot_hide_inside_exact_carrier() {
        let command = commands()[0];
        let mut turn = galley_player_command_turn(&SIGNER, 0, None, command);
        let dregg_turn::Effect::EmitEvent { event, .. } =
            &mut turn.call_forest.roots[0].action.effects[0]
        else {
            panic!("fixture is EmitEvent")
        };
        *event = Event::new(
