...
fn build_actions(env: &Env) -> Vec<Action> {
    ...
    // `composition-governed-test`). The predicate is NOT asserted at verify —
    // predicate verification is fail-closed pending the facts_root AIR binding
    // (see `credentials/src/verification.rs`); the presentation still CARRIES its
    ...
    let verify_opts = VerificationOptions {
        ...
        expected_resource: "composition-governed-test".into(),
        ..Default::default()
    };
    let verify_action =
...
fn submit_all(env: &Env, actions: &[Action]) -> Vec<TurnReceipt> {
    actions
        ...
        .map(|a| {
            ...
            // NON-primary cell (e.g. the governed-namespace propose/vote) then
            // fails the classical half once the clerk's nonce has advanced past
            // the build-time value. The executor's own stale-nonce repair only
            // covers refused-turn nonce DIVERGENCE, not a batch pre-signed at an
            // older nonce — so we stamp the live nonce here, the correct usage for
            // a pre-built batch. (Unrelated to credentials verify(): the credential
            // issue/present/verify actions already commit through the F1 sound
            // path; this repairs the nameservice/governance steps.)
