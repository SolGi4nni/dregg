...
fn build_actions(env: &Env) -> Vec<Action> {
    ...
    // `mandate-composition-test`). The predicate is NOT asserted at verify —
    ...
    let verify_opts = VerificationOptions {
        ...
        expected_resource: "mandate-composition-test".into(),
        ..Default::default()
    };
    let verify_action =
...
fn submit_all(env: &Env, actions: &[Action]) -> Vec<TurnReceipt> {
    actions
        ...
        .map(|a| {
            ...
            // NON-primary cell (e.g. the CWM/SGM mandate cells) then fails the
            ...
            // path; this repairs the mandate-cell steps.)
