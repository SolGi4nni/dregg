#!/usr/bin/env python3
"""Summarize raw paired direct-logic DREGG BatchSTARK samples."""
...
def main() -> None:
    ...
    with open(raw_path, encoding="utf-8") as raw:
        for row in csv.reader(raw):
            if row and row[0] == "SAMPLE":
                samples[(row[1], row[2], row[3])].append(int(row[5]) / 1_000_000)

    print(
        "workload,variant,stage,n,median_ms,p95_ms,mean_ms,stddev_ms,"
    ...
    for seed, (key, values) in enumerate(sorted(samples.items()), 1):
        ...
        print(
            f"{key[0]},{key[1]},{key[2]},{observed.size},"
            f"{median:.6f},{p95:.6f},{np.mean(observed):.6f},"
            f"{np.std(observed, ddof=1):.6f},{median_lo:.6f},{median_hi:.6f},"
            f"{mean_lo:.6f},{mean_hi:.6f},{np.min(observed):.6f},"
            f"{np.max(observed):.6f}"
        )
