...
/// floor (`project-fri-soundness-reality`: no adversary model, so the bit-count is a density
/// reading, not a soundness bound — the theorem-backed deployed columns are
/// `FriDeployedHeightPairing.deployed_wrap_commitBits` (commit) and
/// `DeployedProximitySoundness` (query)). It is a real improvement
