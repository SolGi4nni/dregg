...
/// Canonical field-element encodings of an extended Edwards representative in
/// `(X, Y, Z, T)` order.
///
/// This qualification-only type is exposed for dregg's portable wgpu backend.
/// It is not a second Ristretto serialization format: callers must round-trip
/// GPU output through [`RistrettoPoint::dregg_from_extended_coordinates_checked`]
/// with a canonical expected Ristretto encoding.
#[cfg(feature = "dregg-wgpu-coordinates")]
pub type DreggExtendedCoordinates = [[u8; 32]; 4];

impl RistrettoPoint {
    /// Export this valid Ristretto point's extended Edwards representative as
    /// four canonical field-element encodings.
    #[cfg(feature = "dregg-wgpu-coordinates")]
    pub fn dregg_extended_coordinates(&self) -> DreggExtendedCoordinates {
        [
            self.0.X.as_bytes(),
            self.0.Y.as_bytes(),
            self.0.Z.as_bytes(),
    ...
    /// Import extended Edwards coordinates only after all structural checks
    /// pass and the resulting quotient point compresses to `expected` exactly.
    #[cfg(feature = "dregg-wgpu-coordinates")]
    pub fn dregg_from_extended_coordinates_checked(
        coordinates: &DreggExtendedCoordinates,
        expected: &CompressedRistretto,
    ) -> Option<RistrettoPoint> {
        fn canonical(bytes: &[u8; 32]) -> Option<FieldElement> {
            let element = FieldElement::from_bytes(bytes);
            if element.as_bytes() == *bytes {
                Some(element)
            } else {
                None
            }
        ...
        let X = canonical(&coordinates[0])?;
        let Y = canonical(&coordinates[1])?;
        let Z = canonical(&coordinates[2])?;
        let T = canonical(&coordinates[3])?;
        if bool::from(Z.is_zero()) || &X * &Y != &Z * &T {
            return None;
        }
        ...
        let ZZ = Z.square();
        let left = &(&YY - &XX) * &ZZ;
        let right = &ZZ.square() + &(&constants::EDWARDS_D * &(&XX * &YY));
        if left != right {
            return None;
        }

        let point = RistrettoPoint(EdwardsPoint { X, Y, Z, T });
        if point.compress() == *expected {
            Some(point)
        } else {
            None
        }
    ...
    /// Canonical encoding of Edwards `2*d`, used by the complete extended
    /// Edwards addition formula in the dregg wgpu qualification kernel.
    #[cfg(feature = "dregg-wgpu-coordinates")]
    pub fn dregg_edwards_d2_bytes() -> [u8; 32] {
        constants::EDWARDS_D2.as_bytes()
    }
