...
pub struct RelinKeyShare<R: Round = R1> {
    ...
    // DREGG SEAM (visibility only): widened from private to `pub(crate)` so the
    // additive `share_codec` sibling module can encode and rebuild a share. No
    // value, type or protocol step changes; every construction site below is
    // byte-identical to upstream fhe 0.1.1.
    pub(crate) last_round: Option<Arc<RelinKeyShare<R1Aggregated>>>,
    pub(crate) _phantom_data: PhantomData<R>,
}
