...
//! implementation; the LIVE implementation lives in `dregg-zkoracle-live`
//! (`endpoints::price`), which swaps the fixture for a genuine MPC-TLS 2PC roundtrip against
//! `api.coinbase.com` and reaches the SAME [`price_fact_over_session`] extractor.
//!
...
/// **EXTRACT the price fact from an ALREADY-VERIFIED session** — the half of
/// [`verify_coinbase_spot`] that is downstream of leg 1.
///
/// Parses the asset from the AUTHENTICATED request target and the amount from the
/// AUTHENTICATED body, and cross-checks that the response's `base-currency` is the asset that
/// was actually requested — so the quote is provably about the pair that was asked for.
///
/// `pub` and factored out because the live path (`dregg-zkoracle-live`, whose leg 1 is a real
/// `presentation.verify()`) must reach the SAME extractor. It used to be a verbatim copy
/// behind `#[cfg(feature = "tlsn-live")]`; two copies of a cross-check is one copy too many
/// for the copy to stay right.
pub fn price_fact_over_session(
    att: &ZkOracleAttestation,
    verified: &crate::attestation::VerifiedZkOracle,
) -> Result<AttestedPrice, PriceError> {
